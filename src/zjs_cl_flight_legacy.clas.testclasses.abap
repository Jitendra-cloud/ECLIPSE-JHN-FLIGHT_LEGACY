...
*
*CLASS zjs_cl_flight_legacy DEFINITION LOCAL FRIENDS ltcl_lock_travel.
*
...
*    CONSTANTS mc_msgid                     TYPE symsgid   VALUE 'ZJS_CM_FLIGHT_LEGAC'.
*
...
*
*    CLASS-DATA gr_cut                      TYPE REF TO zjs_cl_flight_legacy.
*
*    CLASS-DATA gv_agency_id_1              TYPE zjs_de_agency_id.
*    CLASS-DATA gv_agency_id_2              TYPE zjs_de_agency_id.
*    CLASS-DATA gv_agency_id_unknown        TYPE zjs_de_agency_id.
*
*    CLASS-DATA gv_customer_id_1            TYPE zjs_de_customer_id.
*    CLASS-DATA gv_customer_id_2            TYPE zjs_de_customer_id.
*    CLASS-DATA gv_customer_id_unknown      TYPE zjs_de_customer_id.
*
*    METHODS _create_travel IMPORTING is_travel        TYPE zjs_st_travel_in
*                                     iv_save          TYPE abap_bool DEFAULT abap_true
*                           RETURNING VALUE(rs_travel) TYPE zjs_travel.
*    METHODS _delete_existing_travel IMPORTING iv_travel_id TYPE zjs_de_travel_id.
*
*  PRIVATE SECTION.
...
*ENDCLASS.
*
*CLASS zjs_cl_flight_legacy DEFINITION LOCAL FRIENDS ltc_travel.
*
*CLASS ltc_travel IMPLEMENTATION.
*  METHOD class_setup.
*    IF mc_use_sql_doubles = abap_true ##BOOL_OK.
**     mr_test_environment = cl_osql_test_environment=>create( i_dependency_list = VALUE #( ( 'ZJS_TRAVEL' ) ( 'ZJS_BOOKING' ) ( 'ZJS_BOOK_SUPPL' )
**                                                                                          ( 'ZJS_AGENCY' ) ( 'ZJS_CUSTOMER' ) ( 'ZJS_FLIGHT' ) ( 'ZJS_SUPPLEMENT' ) ) ).
**     mr_test_environment->clear_doubles( ).
**     gv_agency_id_1 = '42'.
**     gv_agency_id_2 = '43'.
**     DATA lt_agency TYPE STANDARD TABLE OF zjs_agency.
**     lt_agency = VALUE #( ( agency_id = gv_agency_id_1 ) ( agency_id = gv_agency_id_2 ) ).
**     mr_test_environment->insert_test_data( lt_agency ).
**
**     gv_customer_id_1 = '42'.
**     gv_customer_id_2 = '43'.
**     DATA lt_customer  TYPE STANDARD TABLE OF zjs_customer.
...
*    ELSE.
*      DATA lt_agency_id TYPE SORTED TABLE OF zjs_de_agency_id     WITH UNIQUE KEY table_line.
*      SELECT DISTINCT agency_id FROM zjs_agency     ORDER BY agency_id   DESCENDING INTO TABLE @lt_agency_id . "#EC CI_NOWHERE
*
*      DATA lt_customer_id TYPE SORTED TABLE OF zjs_de_customer_id WITH UNIQUE KEY table_line.
*      SELECT DISTINCT customer_id FROM zjs_customer ORDER BY customer_id DESCENDING INTO TABLE @lt_customer_id . "#EC CI_NOWHERE
*
...
*
*  METHOD setup.
*    SELECT COUNT( * ) FROM zjs_travel     INTO @mv_travel_count. "#EC CI_NOWHERE
*    SELECT COUNT( * ) FROM zjs_booking    INTO @mv_booking_count. "#EC CI_NOWHERE
*    SELECT COUNT( * ) FROM zjs_book_suppl INTO @mv_booking_supplement_count. "#EC CI_NOWHERE
*  ENDMETHOD.
*
*
*  METHOD teardown.
*    " Ensure proper cleanup of each individual test method
*    SELECT COUNT( * ) FROM zjs_travel INTO @DATA(lv_travel_count). "#EC CI_NOWHERE
*    cl_abap_unit_assert=>assert_equals( act = lv_travel_count  exp = mv_travel_count ).
*    SELECT COUNT( * ) FROM zjs_booking INTO @DATA(lv_booking_count). "#EC CI_NOWHERE
*    cl_abap_unit_assert=>assert_equals( act = lv_booking_count  exp = mv_booking_count ).
*    SELECT COUNT( * ) FROM zjs_book_suppl INTO @DATA(lv_booking_supplement_count). "#EC CI_NOWHERE
...
*
*  METHOD create.
*    SELECT COUNT( * ) FROM zjs_travel INTO @DATA(lv_count1). "#EC CI_NOWHERE
*
...
*
*    SELECT COUNT( * ) FROM zjs_travel INTO @DATA(lv_count2). "#EC CI_NOWHERE
*    cl_abap_unit_assert=>assert_equals( msg = 'create should add a travel'  exp = 1  act = lv_count2 - lv_count1 ).
*
*    SELECT FROM zjs_travel FIELDS createdby, createdat, status WHERE travel_id = @ls_travel_new-travel_id INTO TABLE @DATA(lt_travel).
...
*
*    cl_abap_unit_assert=>assert_equals( msg = 'status' act = ls_travel-status exp = CONV zjs_de_travel_status( zjs_if_flight_legacy=>travel_status-new ) ).
*
...
*
*    gr_cut->create_travel( EXPORTING is_travel         = VALUE #( agency_id = gv_agency_id_1  customer_id = gv_customer_id_2   begin_date = '20190101'  end_date = '20190201' )
*                                     iv_numbering_mode = zjs_if_flight_legacy=>numbering_mode-late
*                           IMPORTING es_travel         = DATA(ls_travel_new)
*                                     et_messages       = DATA(lt_messages) ).
*    cl_abap_unit_assert=>assert_initial( lt_messages ).
*    cl_abap_unit_assert=>assert_not_initial( ls_travel_new-travel_id ).
...
*    cl_abap_unit_assert=>assert_equals( msg = 'create should add a travel'  exp = 1  act = lv_count2 - lv_count1 ).
*
*    SELECT FROM zjs_travel FIELDS createdby, createdat, status WHERE travel_id = @lv_travel_id_final INTO TABLE @DATA(lt_travel).
*    cl_abap_unit_assert=>assert_equals( msg = 'cannot read created travel' exp = 1 act = lines(  lt_travel ) ).
*    DATA(ls_travel) = lt_travel[ 1 ].
*
*    cl_abap_unit_assert=>assert_equals( msg = 'createdby' exp = ls_travel-createdby act = sy-uname ).
...
*    GET TIME STAMP FIELD lv_end.
*
*    SELECT FROM zjs_travel FIELDS createdby, createdat, status WHERE travel_id = @ls_travel_1-travel_id OR travel_id = @ls_travel_2-travel_id INTO TABLE @DATA(lt_travel) ##SELECT_FAE_WITH_LOB[DESCRIPTION].
...
*      IF lr_message->t100key = zjs_cx_flight_legacy=>agency_unkown.
...
*    DATA lv_msg_found TYPE abap_bool.
*    LOOP AT lt_messages INTO DATA(lr_message) ##INTO_OK.
*      IF lr_message->t100key = zjs_cx_flight_legacy=>customer_unkown.
*        lv_msg_found = abap_true.
*      ENDIF.
*    ENDLOOP.
*    cl_abap_unit_assert=>assert_equals( msg = 'missing error' exp = abap_true act = lv_msg_found ).
...
*
*    SELECT MAX( travel_id ) FROM zjs_travel INTO @DATA(lv_travel_id_max). "#EC CI_NOWHERE
*    DATA lv_travel_id_1 TYPE zjs_de_travel_id.
*    DATA lv_travel_id_2 TYPE zjs_de_travel_id.
...
**                                                                       status = zjs_if_flight_legacy=>travel_status-booked ) )-travel_id  exp = lv_travel_id_1 ).
**
...
**                                                                       status = zjs_if_flight_legacy=>travel_status-booked ) )-travel_id  exp = lv_travel_id_2 ).
*
...
*                                              status = zjs_if_flight_legacy=>travel_status-booked ) )-travel_id.
*
...
*    DATA ls_travel_sel TYPE zjs_travel.
*    SELECT SINGLE * FROM zjs_travel WHERE travel_id = @lv_travel_id_2 INTO @ls_travel_sel.
...
*    cl_abap_unit_assert=>assert_equals( act = ls_travel_sel-status         exp = CONV zjs_de_travel_status( zjs_if_flight_legacy=>travel_status-new ) )." Provided status overridden by determination
...
*
*    SELECT SINGLE description FROM zjs_travel WHERE travel_id = @ls_travel_new-travel_id INTO @DATA(lv_description).
*    cl_abap_unit_assert=>assert_subrc( ).
...
*
*    CLEAR lv_db_exists.
*    SELECT SINGLE FROM zjs_travel FIELDS @abap_true WHERE travel_id = @ls_travel_new-travel_id INTO @lv_db_exists.
...
*
*    DATA ls_travel_sel TYPE zjs_travel.
*    SELECT SINGLE description FROM zjs_travel WHERE travel_id = @ls_travel_new-travel_id INTO CORRESPONDING FIELDS OF @ls_travel_sel.
*    cl_abap_unit_assert=>assert_subrc( ).
...
*
*    CLEAR ls_travel_sel.
*    SELECT SINGLE * FROM zjs_travel WHERE travel_id = @ls_travel_new-travel_id INTO @ls_travel_sel.
*    cl_abap_unit_assert=>assert_subrc( ).
...
*    cl_abap_unit_assert=>assert_true( xsdbool( lt_messages[ 1 ] IS INSTANCE OF zjs_cx_flight_legacy ) ).
*    DATA lx TYPE REF TO zjs_cx_flight_legacy.
*    lx ?= lt_messages[ 1 ].
*    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-msgid  exp = mc_msgid ).
*    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-msgno  exp = zjs_cx_flight_legacy=>travel_unknown-msgno ).
...
*    cl_abap_unit_assert=>assert_initial( lt_messages ).
*
*    DATA lv_description TYPE zjs_de_description.
*
*    SELECT SINGLE description FROM zjs_travel WHERE travel_id = @ls_travel_new_1-travel_id INTO @lv_description.
*    cl_abap_unit_assert=>assert_subrc( ).
*    cl_abap_unit_assert=>assert_equals( act = lv_description  exp = ls_travel_new_1-description ).
*
*    SELECT SINGLE description FROM zjs_travel WHERE travel_id = @ls_travel_new_2-travel_id INTO @lv_description.
*    cl_abap_unit_assert=>assert_subrc( ).
*    cl_abap_unit_assert=>assert_equals( act = lv_description  exp = ls_travel_new_2-description ).
*
*    SELECT SINGLE description FROM zjs_travel WHERE travel_id = @ls_travel_new_3-travel_id INTO @lv_description.
*    cl_abap_unit_assert=>assert_subrc( ).
...
*
*    CLEAR lv_db_exists.
*    SELECT SINGLE FROM zjs_travel FIELDS @abap_true WHERE travel_id IN ( @ls_travel_new_1-travel_id, @ls_travel_new_2-travel_id, @ls_travel_new_3-travel_id ) INTO @lv_db_exists.
*    cl_abap_unit_assert=>assert_false( lv_db_exists ).
*  ENDMETHOD.
*
*
...
*
*    CLEAR ls_travel_sel.
*    SELECT SINGLE agency_id, customer_id, description FROM zjs_travel WHERE travel_id = @ls_travel_new_1-travel_id INTO CORRESPONDING FIELDS OF @ls_travel_sel.
*    cl_abap_unit_assert=>assert_subrc( ).
...
*
*    CLEAR ls_travel_sel.
*    SELECT SINGLE agency_id, customer_id, description FROM zjs_travel WHERE travel_id = @ls_travel_new_2-travel_id INTO CORRESPONDING FIELDS OF @ls_travel_sel.
*    cl_abap_unit_assert=>assert_subrc( ).
...
*
*    CLEAR ls_travel_sel.
*    SELECT SINGLE agency_id, customer_id, description FROM zjs_travel WHERE travel_id = @ls_travel_new_3-travel_id INTO CORRESPONDING FIELDS OF @ls_travel_sel.
*    cl_abap_unit_assert=>assert_subrc( ).
*    cl_abap_unit_assert=>assert_equals( act = ls_travel_sel-agency_id    exp = gv_agency_id_2   ).
...
*
*    CLEAR ls_travel_sel.
*    SELECT SINGLE agency_id, customer_id, description FROM zjs_travel WHERE travel_id = @ls_travel_new-travel_id INTO CORRESPONDING FIELDS OF @ls_travel_sel.
*    cl_abap_unit_assert=>assert_subrc( ).
*    cl_abap_unit_assert=>assert_equals( act = ls_travel_sel-agency_id    exp = gv_agency_id_2 ).
*    cl_abap_unit_assert=>assert_equals( act = ls_travel_sel-customer_id  exp = ls_travel_new-customer_id ).
*    cl_abap_unit_assert=>assert_equals( act = ls_travel_sel-description  exp = 'My_New_Text' ).
...
*    lx ?= lt_messages[ 1 ].
*    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-msgid  exp = mc_msgid ).
*    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-msgno  exp = zjs_cx_flight_legacy=>agency_unkown-msgno ).
...
*    lx ?= lt_messages[ 1 ].
*    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-msgid  exp = mc_msgid ).
*    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-msgno  exp = zjs_cx_flight_legacy=>customer_unkown-msgno ).
...
*    lx ?= lt_messages[ 1 ].
*    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-msgid  exp = mc_msgid ).
*    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-msgno  exp = zjs_cx_flight_legacy=>travel_no_key-msgno ).
*  ENDMETHOD.
*
*
...
*
*    DATA ls_travel_sel TYPE zjs_travel.
*    SELECT SINGLE status FROM zjs_travel WHERE travel_id = @ls_travel_new-travel_id INTO CORRESPONDING FIELDS OF @ls_travel_sel.
*    cl_abap_unit_assert=>assert_subrc( ).
*    cl_abap_unit_assert=>assert_equals( act = ls_travel_sel-status  exp = CONV zjs_de_travel_status( zjs_if_flight_legacy=>travel_status-new ) ).
*
...
*
*    CLEAR ls_travel_sel.
*    SELECT SINGLE status, createdat, lastchangedby, lastchangedat FROM zjs_travel WHERE travel_id = @ls_travel_new-travel_id INTO CORRESPONDING FIELDS OF @ls_travel_sel.
*    cl_abap_unit_assert=>assert_subrc( ).
*    cl_abap_unit_assert=>assert_equals( act = ls_travel_sel-status  exp = CONV zjs_de_travel_status( zjs_if_flight_legacy=>travel_status-booked ) ).
*    cl_abap_unit_assert=>assert_equals( act = ls_travel_sel-lastchangedby  exp = sy-uname ).
*    cl_abap_unit_assert=>assert_differs( act = ls_travel_sel-createdat  exp = ls_travel_sel-lastchangedat ).
*    DATA(lv_diff) = cl_abap_tstmp=>subtract( tstmp1 = ls_travel_sel-lastchangedat  tstmp2 = lv_timestampl ).
*    cl_abap_unit_assert=>assert_true( xsdbool( 0 < lv_diff AND lv_diff < 1 ) ).
...
*    lx ?= lt_messages[ 1 ].
*    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-msgid  exp = mc_msgid ).
*    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-msgno  exp = zjs_cx_flight_legacy=>no_begin_date-msgno ).
*    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-attr1  exp = 'MV_TRAVEL_ID' ).
*
...
*    lx ?= lt_messages[ 1 ].
*    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-msgid  exp = mc_msgid ).
*    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-msgno  exp = zjs_cx_flight_legacy=>no_end_date-msgno ).
*    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-attr1  exp = 'MV_TRAVEL_ID' ).
*
...
*    lx ?= lt_messages[ 1 ].
*    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-msgid  exp = mc_msgid ).
*    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-msgno  exp = zjs_cx_flight_legacy=>end_date_before_begin_date-msgno ).
...
*    gr_cut->save( ).
*
*    SELECT SINGLE agency_id, description FROM zjs_travel WHERE travel_id = @ls_travel-travel_id INTO ( @DATA(lv_agency_id), @DATA(lv_description) ).
*    cl_abap_unit_assert=>assert_subrc( ).
...
*
*    CLEAR lv_db_exits.
*    SELECT SINGLE FROM zjs_travel FIELDS @abap_true WHERE travel_id = @ls_travel-travel_id INTO @lv_db_exits.
...
*
*    CLEAR lv_db_exits.
*    SELECT SINGLE FROM zjs_travel FIELDS @abap_true WHERE travel_id = @lv_travel_id INTO @lv_db_exits.
*    cl_abap_unit_assert=>assert_false( lv_db_exits ).
*  ENDMETHOD.
*
*
...
*    lx ?= lt_messages[ 1 ].
*    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-msgid  exp = mc_msgid ).
*    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-msgno  exp = zjs_cx_flight_legacy=>travel_no_control-msgno ).
*    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-attr1  exp = 'MV_TRAVEL_ID' ).
*    cl_abap_unit_assert=>assert_equals( act = lx->mv_travel_id                   exp = ls_travel-travel_id ).
*
*    _delete_existing_travel( ls_travel-travel_id ).
...
*    CONSTANTS lc_status_invalid TYPE zjs_de_travel_status VALUE 'Z'.
*
*    DATA(ls_travel) = _create_travel( VALUE #( agency_id = gv_agency_id_1  customer_id = gv_customer_id_1  begin_date = '20190101'  end_date = '20190201' ) ).
*
*    " No control data at all
...
*    lx ?= lt_messages[ 1 ].
*    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-msgid  exp = mc_msgid ).
*    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-msgno  exp = zjs_cx_flight_legacy=>travel_status_invalid-msgno ).
...
*    "Test Travel IDs
*    CONSTANTS cv_travel_id_initial          TYPE zjs_de_travel_id VALUE IS INITIAL.
*    CONSTANTS cv_travel_id_in_delete_buffer TYPE zjs_de_travel_id VALUE '123'.
*    CONSTANTS cv_travel_id_in_create_buffer TYPE zjs_de_travel_id VALUE '321'.
*    DATA      lv_travel_id_in_db            TYPE zjs_de_travel_id.
*    DATA      lv_travel_id_unknown          TYPE zjs_de_travel_id.
*
*    "Other variables
*    DATA lv_travel_id_is_valid TYPE abap_bool.
*    DATA: lt_messages TYPE zjs_if_flight_legacy=>tt_if_t100_message.
*
*    "setup
...
*    SELECT SINGLE FROM zjs_travel FIELDS MAX( travel_id ) INTO @lv_travel_id_in_db.
...
*    CONSTANTS cv_travel_id_early       TYPE zjs_de_travel_id VALUE '42'.
*    DATA      lv_travel_id_preliminary TYPE zjs_de_travel_id.
*
*    DATA lt_mapping TYPE zjs_if_flight_legacy=>tt_ln_travel_mapping.
*
*    DATA(lo_buffer) = NEW lcl_travel_buffer( ).
*
*    "early
...
*    CLEAR lt_mapping.
*    CLEAR lo_buffer->mt_create_buffer.
*    lv_travel_id_preliminary = zjs_if_flight_legacy=>late_numbering_boundary + 1.
...
*             carrier_id    TYPE zjs_de_carrier_id,
*             connection_id TYPE zjs_de_connection_id,
*             flight_date   TYPE zjs_de_flight_date,
*             price         TYPE zjs_de_flight_price,
*             currency_code TYPE zjs_de_currency_code,
*           END OF ts_flight.
*
*    CLASS-DATA gs_flight_1 TYPE ts_flight.
*    CLASS-DATA gs_flight_2 TYPE ts_flight.
*
*    CLASS-DATA gv_carrier_id_unknown       TYPE zjs_de_carrier_id.
*
*    CLASS-DATA gv_currency_code_unknown    TYPE zjs_de_currency_code.
*
*    CLASS-DATA gv_booking_date             TYPE zjs_de_booking_date.
*
*    DATA mv_travel_id         TYPE zjs_de_travel_id.
*    DATA mv_travel_id_unknown TYPE zjs_de_travel_id.
*
*    METHODS _create_booking IMPORTING iv_travel_id      TYPE zjs_de_travel_id
*                                      is_booking        TYPE zjs_st_booking_in
*                                      iv_save           TYPE abap_bool DEFAULT abap_true
*                            RETURNING VALUE(rs_booking) TYPE zjs_booking.
*    METHODS _delete_existing_booking IMPORTING iv_travel_id  TYPE zjs_de_travel_id
*                                               iv_booking_id TYPE zjs_de_booking_id.
*
*  PRIVATE SECTION.
*    CLASS-METHODS class_setup.
*    METHODS setup.
...
*ENDCLASS.
*
*CLASS zjs_cl_flight_legacy DEFINITION LOCAL FRIENDS ltc_booking.
*
...
**
**     DATA lt_flight TYPE STANDARD TABLE OF zjs_flight.
...
*      SELECT SINGLE carrier_id, connection_id, flight_date, price, currency_code FROM zjs_flight
*        INTO CORRESPONDING FIELDS OF @gs_flight_1 ##WARN_OK. "#EC CI_NOORDER
*      IF sy-subrc <> 0.
*        cl_abap_unit_assert=>abort( |No flight data!| ).
*      ENDIF.
*      SELECT SINGLE carrier_id, connection_id, flight_date, price, currency_code FROM zjs_flight WHERE carrier_id <> @gs_flight_1-carrier_id AND connection_id <> @gs_flight_1-connection_id
...
*      SELECT SINGLE carrier_id FROM zjs_carrier WHERE carrier_id = @gv_carrier_id_unknown INTO @DATA(lv_dummy_carrier_id).
*      IF sy-subrc = 0.
...
*      SELECT SINGLE travel_id FROM zjs_travel WHERE travel_id = @mv_travel_id_unknown INTO @DATA(lv_dummy_travel_id).
*      IF sy-subrc <> 0.
*        EXIT.
*      ENDIF.
*    ENDDO.
...
*
*    DATA ls_booking_sel TYPE zjs_booking.
*    SELECT SINGLE customer_id FROM zjs_booking WHERE travel_id = @mv_travel_id AND booking_id = @ls_booking-booking_id INTO CORRESPONDING FIELDS OF @ls_booking_sel.
*    cl_abap_unit_assert=>assert_subrc( ).
...
*                                     it_bookingx = VALUE #( ( booking_id = ls_booking-booking_id  action_code = zjs_if_flight_legacy=>action_code-update  booking_date = abap_true ) )
*                           IMPORTING et_messages = DATA(lt_messages) ).
*    cl_abap_unit_assert=>assert_initial( lt_messages ).
*    gr_cut->save( ).
*
*    CLEAR ls_booking_sel.
*    SELECT SINGLE carrier_id, booking_date FROM zjs_booking WHERE travel_id = @mv_travel_id AND booking_id = @ls_booking-booking_id INTO CORRESPONDING FIELDS OF @ls_booking_sel.
*    cl_abap_unit_assert=>assert_subrc( ).
...
*    SELECT SINGLE @abap_true FROM zjs_booking WHERE travel_id = @mv_travel_id AND booking_id = @ls_booking-booking_id INTO @DATA(lv_found).
...
*    DATA lv_db_exists TYPE abap_bool.
*
*    SELECT SINGLE lastchangedat FROM zjs_travel WHERE travel_id = @mv_travel_id INTO @DATA(lv_lastchangedat_1).
*
*    DATA(ls_booking) = _create_booking( iv_travel_id = mv_travel_id
*                                        is_booking   = VALUE #( booking_id    = '10'
*                                                                booking_date  = gv_booking_date
...
*                                        iv_save      = abap_false ).
*
*    DATA lv_booking_date TYPE zjs_de_booking_date.
...
*                                     it_bookingx = VALUE #( ( booking_id = ls_booking-booking_id  action_code = zjs_if_flight_legacy=>action_code-delete ) )
*                           IMPORTING et_messages = lt_messages ).
*    cl_abap_unit_assert=>assert_initial( lt_messages ).
*
*    gr_cut->save( ).
...
*    SELECT SINGLE lastchangedby, lastchangedat FROM zjs_travel WHERE travel_id = @mv_travel_id INTO ( @DATA(lv_lastchangedby), @DATA(lv_lastchangedat_2) ).
...
*
*    CLEAR lv_db_exists.
*    SELECT SINGLE FROM zjs_booking FIELDS @abap_true WHERE travel_id = @mv_travel_id AND booking_id = @ls_booking-booking_id INTO @lv_db_exists.
*    cl_abap_unit_assert=>assert_false( lv_db_exists ).
*  ENDMETHOD.
*
*
...
*                                                                flight_date   = gs_flight_1-flight_date ) ).
*
*    SELECT SINGLE customer_id FROM zjs_booking WHERE travel_id = @mv_travel_id AND booking_id = @ls_booking-booking_id INTO @DATA(lv_customer_id).
*    cl_abap_unit_assert=>assert_subrc( ).
...
*                                     it_bookingx = VALUE #( ( booking_id = ls_booking-booking_id  action_code = zjs_if_flight_legacy=>action_code-update  customer_id = abap_true ) )
*                           IMPORTING et_messages = lt_messages ).
*    cl_abap_unit_assert=>assert_initial( lt_messages ).
*    gr_cut->save( ).
*
*    DATA ls_booking_sel TYPE zjs_booking.
*    SELECT SINGLE booking_date, customer_id FROM zjs_booking WHERE travel_id = @mv_travel_id AND booking_id = @ls_booking-booking_id INTO CORRESPONDING FIELDS OF @ls_booking_sel.
*    cl_abap_unit_assert=>assert_subrc( ).
*    cl_abap_unit_assert=>assert_equals( act = ls_booking_sel-booking_date  exp = lv_booking_date ).
...
*                                     it_bookingx = VALUE #( ( booking_id = '10'  action_code = zjs_if_flight_legacy=>action_code-create ) )
...
*                                     it_bookingx = VALUE #( ( booking_id = '2'  action_code = zjs_if_flight_legacy=>action_code-update  booking_date = abap_true ) )
*                           IMPORTING et_messages = DATA(lt_messages) ).
*    cl_abap_unit_assert=>assert_equals( act = lines( lt_messages )  exp = 1 ).
*    cl_abap_unit_assert=>assert_true( xsdbool( lt_messages[ 1 ] IS INSTANCE OF zjs_cx_flight_legacy ) ).
*    DATA lx TYPE REF TO zjs_cx_flight_legacy.
...
*                                     it_bookingx = VALUE #( ( booking_id = '1'  action_code = zjs_if_flight_legacy=>action_code-delete ) )
*                           IMPORTING et_messages = DATA(lt_messages) ).
*    cl_abap_unit_assert=>assert_equals( act = lines( lt_messages )  exp = 1 ).
*    cl_abap_unit_assert=>assert_true( xsdbool( lt_messages[ 1 ] IS INSTANCE OF zjs_cx_flight_legacy ) ).
*    DATA lx TYPE REF TO zjs_cx_flight_legacy.
...
*                                     it_bookingx = VALUE #( ( booking_id = '10'  action_code = zjs_if_flight_legacy=>action_code-update  booking_date = abap_true  ) )
*                           IMPORTING et_messages = DATA(lt_messages) ).
*    cl_abap_unit_assert=>assert_equals( act = lines( lt_messages )  exp = 1 ).
*    cl_abap_unit_assert=>assert_true( xsdbool( lt_messages[ 1 ] IS INSTANCE OF zjs_cx_flight_legacy ) ).
*    DATA lx TYPE REF TO zjs_cx_flight_legacy.
*    lx ?= lt_messages[ 1 ].
*    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-msgid  exp = mc_msgid ).
*    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-msgno  exp = zjs_cx_flight_legacy=>booking_unknown-msgno ).
*    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-attr1  exp = 'MV_TRAVEL_ID' ).
...
*                                     it_bookingx = VALUE #( ( booking_id = '10'  action_code = zjs_if_flight_legacy=>action_code-delete ) )
*                           IMPORTING et_messages = DATA(lt_messages) ).
*    cl_abap_unit_assert=>assert_equals( act = lines( lt_messages )  exp = 1 ).
*    cl_abap_unit_assert=>assert_true( xsdbool( lt_messages[ 1 ] IS INSTANCE OF zjs_cx_flight_legacy ) ).
*    DATA lx TYPE REF TO zjs_cx_flight_legacy.
...
*                                     it_bookingx = VALUE #( ( booking_id = '0'  action_code = zjs_if_flight_legacy=>action_code-update  booking_date = abap_true ) )
*                           IMPORTING et_messages = DATA(lt_messages) ).
*    cl_abap_unit_assert=>assert_equals( act = lines( lt_messages )  exp = 1 ).
*    cl_abap_unit_assert=>assert_true( xsdbool( lt_messages[ 1 ] IS INSTANCE OF zjs_cx_flight_legacy ) ).
*    DATA lx TYPE REF TO zjs_cx_flight_legacy.
*    lx ?= lt_messages[ 1 ].
*    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-msgid  exp = mc_msgid ).
*    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-msgno  exp = zjs_cx_flight_legacy=>booking_no_key-msgno ).
*    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-attr1  exp = 'MV_TRAVEL_ID' ).
*    cl_abap_unit_assert=>assert_equals( act = lx->mv_travel_id                   exp = mv_travel_id ).
*  ENDMETHOD.
*
...
*                                     it_bookingx = VALUE #( ( booking_id = '0'  action_code = zjs_if_flight_legacy=>action_code-delete ) )
*                           IMPORTING et_messages = DATA(lt_messages) ).
*    cl_abap_unit_assert=>assert_equals( act = lines( lt_messages )  exp = 1 ).
*    cl_abap_unit_assert=>assert_true( xsdbool( lt_messages[ 1 ] IS INSTANCE OF zjs_cx_flight_legacy ) ).
*    DATA lx TYPE REF TO zjs_cx_flight_legacy.
...
*    gr_cut->save( ).
*
*    SELECT COUNT( * ) FROM zjs_booking WHERE travel_id = @ls_travel_1-travel_id INTO @DATA(lv_count).
...
*                                     it_bookingx = VALUE #( ( booking_id = ls_booking_1-booking_id  action_code = zjs_if_flight_legacy=>action_code-update  customer_id = abap_true ) )
*                           IMPORTING et_messages = DATA(lt_messages) ).
*    cl_abap_unit_assert=>assert_initial( lt_messages ).
*
*    gr_cut->save( ).
*
*    SELECT SINGLE customer_id FROM zjs_booking WHERE travel_id = @ls_booking_1-travel_id AND booking_id = @ls_booking_1-booking_id INTO @DATA(lv_customer_id).
*    cl_abap_unit_assert=>assert_subrc( ).
*    cl_abap_unit_assert=>assert_equals( act = lv_customer_id  exp = gv_customer_id_2 ).
*
*    SELECT SINGLE customer_id FROM zjs_booking WHERE travel_id = @ls_booking_2-travel_id AND booking_id = @ls_booking_2-booking_id INTO @lv_customer_id.
*    cl_abap_unit_assert=>assert_subrc( ).
...
*
*    SELECT travel_id, booking_id FROM zjs_booking WHERE travel_id = @ls_travel_1-travel_id INTO TABLE @DATA(lt_booking_key).
...
*
*    CLEAR lv_db_exists.
*    SELECT SINGLE FROM zjs_booking FIELDS @abap_true WHERE travel_id = @ls_travel_1-travel_id INTO @lv_db_exists.
*    cl_abap_unit_assert=>assert_false( lv_db_exists ).
*  ENDMETHOD.
*
*
...
*                                                              action_code = zjs_if_flight_legacy=>action_code-create ) )
*                           IMPORTING et_booking  = DATA(lt_booking)
*                                     et_messages = DATA(lt_messages) ).
*    cl_abap_unit_assert=>assert_initial( lt_booking ).
*    cl_abap_unit_assert=>assert_equals( act = lines( lt_messages )  exp = 1 ).
...
*    lx ?= lt_messages[ 1 ].
*    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-msgid  exp = mc_msgid ).
*    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-msgno  exp = zjs_cx_flight_legacy=>booking_booking_date_invalid-msgno ).
*    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-attr1  exp = 'MV_TRAVEL_ID' ).
*    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-attr2  exp = 'MV_BOOKING_ID' ).
...
*    gr_cut->save( ).
*
*    SELECT SINGLE booking_date FROM zjs_booking WHERE travel_id = @mv_travel_id AND booking_id = @ls_booking-booking_id INTO @DATA(lv_booking_date).
*    cl_abap_unit_assert=>assert_subrc( ).
...
*    lx ?= lt_messages[ 1 ].
*    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-msgid  exp = mc_msgid ).
*    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-msgno  exp = zjs_cx_flight_legacy=>flight_unknown-msgno ).
...
*                                     it_bookingx = VALUE #( ( booking_id = ls_booking-booking_id  action_code = zjs_if_flight_legacy=>action_code-update  flight_date = abap_true ) )
*                           IMPORTING et_messages = DATA(lt_messages) ).
*    cl_abap_unit_assert=>assert_equals( act = lines( lt_messages )  exp = 1 ).
*    cl_abap_unit_assert=>assert_true( xsdbool( lt_messages[ 1 ] IS INSTANCE OF zjs_cx_flight_legacy ) ).
*    DATA lx TYPE REF TO zjs_cx_flight_legacy.
*    lx ?= lt_messages[ 1 ].
*    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-msgid  exp = mc_msgid ).
*    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-msgno  exp = zjs_cx_flight_legacy=>booking_flight_u-msgno ).
*    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-attr1  exp = 'MV_TRAVEL_ID' ).
*    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-attr2  exp = 'MV_BOOKING_ID' ).
*    cl_abap_unit_assert=>assert_equals( act = lx->mv_travel_id                   exp = mv_travel_id ).
*    cl_abap_unit_assert=>assert_equals( act = lx->mv_booking_id                  exp = '10' ).
...
*    gr_cut->save( ).
*
*    SELECT SINGLE flight_date FROM zjs_booking WHERE travel_id = @mv_travel_id AND booking_id = @ls_booking-booking_id INTO @DATA(lv_flight_date).
*    cl_abap_unit_assert=>assert_subrc( ).
...
*    DATA(ls_travel) = _create_travel( VALUE #( agency_id = gv_agency_id_1  customer_id = gv_customer_id_1  begin_date = '20190101'  end_date = '20190201' ) ).
*    CLEAR lv_db_exists.
*    SELECT SINGLE FROM zjs_travel FIELDS @abap_true WHERE travel_id = @ls_travel-travel_id INTO @lv_db_exists.
...
*
*    CLEAR lv_db_exists.
*    SELECT SINGLE FROM zjs_booking FIELDS @abap_true WHERE travel_id = @ls_travel-travel_id INTO @lv_db_exists.
*    cl_abap_unit_assert=>assert_false( lv_db_exists ).
*  ENDMETHOD.
*
*
...
*    lx ?= lt_messages[ 1 ].
*    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-msgid  exp = mc_msgid ).
*    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-msgno  exp = zjs_cx_flight_legacy=>booking_exists-msgno ).
*    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-attr1  exp = 'MV_TRAVEL_ID' ).
*    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-attr2  exp = 'MV_BOOKING_ID' ).
*    cl_abap_unit_assert=>assert_equals( act = lx->mv_travel_id                   exp = mv_travel_id ).
*    cl_abap_unit_assert=>assert_equals( act = lx->mv_booking_id                  exp = '10' ).
...
*    DATA lv_flight_price  TYPE zjs_de_flight_price.
*    DATA lv_currency_code TYPE zjs_de_currency_code.
...
*
*    DATA ls_booking_sel TYPE zjs_booking.
*    SELECT SINGLE flight_price, currency_code FROM zjs_booking WHERE travel_id = @ls_booking_1-travel_id AND booking_id = @ls_booking_1-booking_id INTO CORRESPONDING FIELDS OF @ls_booking_sel.
*    cl_abap_unit_assert=>assert_subrc( ).
...
*
*    CLEAR ls_booking_sel.
*    SELECT SINGLE flight_price, currency_code FROM zjs_booking WHERE travel_id = @ls_booking_1-travel_id AND booking_id = @ls_booking_2-booking_id INTO CORRESPONDING FIELDS OF @ls_booking_sel.
*    cl_abap_unit_assert=>assert_subrc( ).
...
*                                     it_bookingx = VALUE #( ( booking_id = ls_booking-booking_id  action_code = zjs_if_flight_legacy=>action_code-update  flight_price = abap_true ) )
*                           IMPORTING et_messages = DATA(lt_messages) ).
*    cl_abap_unit_assert=>assert_equals( act = lines( lt_messages )  exp = 1 ).
*    cl_abap_unit_assert=>assert_true( xsdbool( lt_messages[ 1 ] IS INSTANCE OF zjs_cx_flight_legacy ) ).
*    DATA lx TYPE REF TO zjs_cx_flight_legacy.
*    lx ?= lt_messages[ 1 ].
*    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-msgid  exp = mc_msgid ).
*    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-msgno  exp = zjs_cx_flight_legacy=>booking_price_currency_u-msgno ).
*    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-attr1  exp = 'MV_TRAVEL_ID' ).
*    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-attr2  exp = 'MV_BOOKING_ID' ).
...
*                                     it_bookingx = VALUE #( ( booking_id = ls_booking-booking_id  action_code = zjs_if_flight_legacy=>action_code-update  currency_code = abap_true ) )
*                           IMPORTING et_messages = lt_messages ).
*    cl_abap_unit_assert=>assert_equals( act = lines( lt_messages )  exp = 1 ).
*    cl_abap_unit_assert=>assert_true( xsdbool( lt_messages[ 1 ] IS INSTANCE OF zjs_cx_flight_legacy ) ).
*    lx ?= lt_messages[ 1 ].
...
*    lx ?= lt_messages[ 1 ].
*    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-msgid  exp = mc_msgid ).
*    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-msgno  exp = zjs_cx_flight_legacy=>currency_unknown-msgno ).
...
*                                     it_bookingx = VALUE #( ( booking_id = ls_booking-booking_id  action_code = zjs_if_flight_legacy=>action_code-update  flight_price = abap_true  currency_code = abap_true ) )
*                           IMPORTING et_messages = DATA(lt_messages) ).
*    cl_abap_unit_assert=>assert_equals( act = lines( lt_messages )  exp = 1 ).
*    cl_abap_unit_assert=>assert_true( xsdbool( lt_messages[ 1 ] IS INSTANCE OF zjs_cx_flight_legacy ) ).
*    DATA lx TYPE REF TO zjs_cx_flight_legacy.
...
*                                     it_bookingx = VALUE #( ( booking_id = is_booking-booking_id  action_code = zjs_if_flight_legacy=>action_code-create ) )
*                           IMPORTING et_booking  = DATA(lt_booking)
*                                     et_messages = DATA(lt_messages) ).
*    cl_abap_unit_assert=>assert_initial( lt_messages ).
...
*                                     it_bookingx = VALUE #( ( booking_id = iv_booking_id  action_code = zjs_if_flight_legacy=>action_code-delete ) )
*                           IMPORTING et_messages = DATA(lt_messages) ).
*    cl_abap_unit_assert=>assert_initial( lt_messages ).
*    gr_cut->save( ).
*  ENDMETHOD.
*
*  METHOD adjust_numbers.
*    CONSTANTS cv_travel_id_preliminary_1 TYPE zjs_de_travel_id VALUE '1337'.
*    CONSTANTS cv_travel_id_final_1       TYPE zjs_de_travel_id VALUE '42'.
*    CONSTANTS cv_travel_id_final_2       TYPE zjs_de_travel_id VALUE '314'.
*    CONSTANTS cv_booking_id_final_1      TYPE zjs_de_booking_id VALUE '1'.
*    CONSTANTS cv_booking_id_final_2      TYPE zjs_de_booking_id VALUE '2'.
*
*    DATA lt_mapping TYPE zjs_if_flight_legacy=>tt_ln_booking_mapping.
*
...
*                ]-final
*        exp = VALUE zjs_if_flight_legacy=>ts_ln_booking(
...
*             supplement_id TYPE zjs_de_supplement_id,
*             price         TYPE zjs_de_supplement_price,
*             currency_code TYPE zjs_de_currency_code,
...
*
*    CLASS-DATA gv_supplement_id_unknown      TYPE zjs_de_supplement_id.
*
*    DATA mv_booking_id         TYPE zjs_de_booking_id.
*    DATA mv_booking_id_unknown TYPE zjs_de_booking_id.
*
*    CLASS-METHODS class_setup.
*    METHODS setup.
*
...
*    METHODS u_currency_code_unknown  FOR TESTING RAISING cx_static_check.
*
*    METHODS _create_booking_suppl IMPORTING iv_travel_id                 TYPE zjs_de_travel_id
*                                            is_booking_supplement        TYPE zjs_st_booking_supplement_in
*                                            iv_save                      TYPE abap_bool DEFAULT abap_true
*                                  RETURNING VALUE(rs_booking_supplement) TYPE zjs_book_suppl.
*
...
*ENDCLASS.
*
*CLASS zjs_cl_flight_legacy DEFINITION LOCAL FRIENDS ltc_booking_supplement.
*
...
**
**     DATA lt_supplement TYPE STANDARD TABLE OF zjs_supplement.
...
*      SELECT SINGLE supplement_id, price, currency_code FROM zjs_supplement INTO ( @gs_supplement_1-supplement_id, @gs_supplement_1-price, @gs_supplement_1-currency_code ) ##WARN_OK. "#EC CI_NOORDER
*      IF sy-subrc <> 0.
*        cl_abap_unit_assert=>abort( 'No supplement data!' ).
*      ENDIF.
*
*      " Select a different valid combination of Supplement ID, Price, Currency Code
*      SELECT SINGLE supplement_id, price, currency_code FROM zjs_supplement WHERE supplement_id <> @gs_supplement_1-supplement_id
...
*      gv_supplement_id_unknown = 'XX-999'.
*      SELECT SINGLE supplement_id FROM zjs_supplement WHERE supplement_id = @gv_supplement_id_unknown INTO @DATA(lv_dummy_supplement_id).
*      IF sy-subrc = 0.
...
*    gr_cut->save( ).
*
*    SELECT COUNT( * ) FROM zjs_book_suppl WHERE travel_id = @ls_travel-travel_id INTO @DATA(lv_count).
*    cl_abap_unit_assert=>assert_equals( act = lv_count  exp = 3 ).
*
*    DATA ls_book_suppl_sel TYPE zjs_book_suppl.
*
*    SELECT SINGLE price, currency_code FROM zjs_book_suppl WHERE travel_id             = @ls_book_suppl_2_1-travel_id
...
*                                                                         booking_supplement_id = ls_book_suppl_2_1-booking_supplement_id
*                                                                         action_code           = zjs_if_flight_legacy=>action_code-update
*                                                                         price                 = abap_true
...
*    cl_abap_unit_assert=>assert_equals( act = ls_book_suppl_sel-currency_code  exp = 'EUR' ).
*
*    SELECT SINGLE price, currency_code FROM zjs_book_suppl WHERE travel_id             = @ls_book_suppl_2_2-travel_id
...
*    _delete_existing_booking( iv_travel_id = ls_booking_1-travel_id  iv_booking_id = ls_booking_1-booking_id ).
*
*    SELECT COUNT( * ) FROM zjs_book_suppl WHERE travel_id = @ls_travel-travel_id INTO @lv_count.
*    cl_abap_unit_assert=>assert_equals( act = lv_count  exp = 2 ).
*
*    SELECT COUNT( * ) FROM zjs_book_suppl WHERE travel_id = @ls_travel-travel_id AND booking_id = @ls_booking_2-booking_id INTO @lv_count.
*    cl_abap_unit_assert=>assert_equals( act = lv_count  exp = 2 ).
*
*    " 4. LUW: Delete the second booking supplement of the second booking - here check also travel admin field
*    SELECT SINGLE lastchangedat FROM zjs_travel WHERE travel_id = @ls_travel-travel_id INTO @DATA(lv_lastchangedat_1).
*    cl_abap_unit_assert=>assert_subrc( ).
*
*    CLEAR lv_db_exists.
*    SELECT SINGLE FROM zjs_book_suppl FIELDS @abap_true WHERE travel_id             = @ls_book_suppl_2_2-travel_id
*                                                           AND booking_id            = @ls_book_suppl_2_2-booking_id
...
*                                                                         booking_supplement_id = ls_book_suppl_2_2-booking_supplement_id
*                                                                         action_code = zjs_if_flight_legacy=>action_code-delete ) )
*                           IMPORTING et_messages            = lt_messages ).
*    cl_abap_unit_assert=>assert_initial( lt_messages ).
*    gr_cut->save( ).
*
...
*    cl_abap_unit_assert=>assert_false( lv_db_exists ).
*
*    SELECT SINGLE lastchangedat FROM zjs_travel WHERE travel_id = @ls_travel-travel_id INTO @DATA(lv_lastchangedat_2).
*    cl_abap_unit_assert=>assert_subrc( ).
*
*    cl_abap_unit_assert=>assert_true( xsdbool( lv_lastchangedat_2 > lv_lastchangedat_1 ) ).
*
...
*
*    CLEAR lv_db_exists.
*    SELECT SINGLE FROM zjs_book_suppl FIELDS @abap_true WHERE travel_id = @ls_travel-travel_id INTO @lv_db_exists.
*    cl_abap_unit_assert=>assert_false( lv_db_exists ).
*  ENDMETHOD.
*
*
...
*                                     it_booking_supplementx = VALUE #( ( booking_id = '2'  action_code = zjs_if_flight_legacy=>action_code-update  price = abap_true ) )
*                           IMPORTING et_messages            = DATA(lt_messages) ).
*    cl_abap_unit_assert=>assert_equals( act = lines( lt_messages )  exp = 1 ).
*    cl_abap_unit_assert=>assert_true( xsdbool( lt_messages[ 1 ] IS INSTANCE OF zjs_cx_flight_legacy ) ).
*    DATA lx TYPE REF TO zjs_cx_flight_legacy.
...
*                                     it_booking_supplementx = VALUE #( ( booking_id = '2'  booking_supplement_id = '1'  action_code = zjs_if_flight_legacy=>action_code-update  price = abap_true ) )
*                           IMPORTING et_messages            = DATA(lt_messages) ).
*    cl_abap_unit_assert=>assert_equals( act = lines( lt_messages )  exp = 1 ).
*    cl_abap_unit_assert=>assert_true( xsdbool( lt_messages[ 1 ] IS INSTANCE OF zjs_cx_flight_legacy ) ).
*    DATA lx TYPE REF TO zjs_cx_flight_legacy.
...
*                                     it_booking_supplementx = VALUE #( ( booking_id = '1'  booking_supplement_id = '1'  action_code = zjs_if_flight_legacy=>action_code-delete ) )
*                           IMPORTING et_messages            = DATA(lt_messages) ).
*    cl_abap_unit_assert=>assert_equals( act = lines( lt_messages )  exp = 1 ).
*    cl_abap_unit_assert=>assert_true( xsdbool( lt_messages[ 1 ] IS INSTANCE OF zjs_cx_flight_legacy ) ).
*    DATA lx TYPE REF TO zjs_cx_flight_legacy.
...
*                                     it_booking_supplementx = VALUE #( ( booking_supplement_id = '1'  action_code = zjs_if_flight_legacy=>action_code-create ) )
*                           IMPORTING et_booking_supplement  = DATA(lt_booking_supplement)
*                                     et_messages            = DATA(lt_messages) ).
*    cl_abap_unit_assert=>assert_initial( lt_booking_supplement ).
*    cl_abap_unit_assert=>assert_equals( act = lines( lt_messages )  exp = 1 ).
...
*                                                                         action_code = zjs_if_flight_legacy=>action_code-update  price = abap_true ) )
*                           IMPORTING et_messages            = DATA(lt_messages) ).
*    cl_abap_unit_assert=>assert_equals( act = lines( lt_messages )  exp = 1 ).
*    cl_abap_unit_assert=>assert_true( xsdbool( lt_messages[ 1 ] IS INSTANCE OF zjs_cx_flight_legacy ) ).
*    DATA lx TYPE REF TO zjs_cx_flight_legacy.
...
*                                     it_booking_supplementx = VALUE #( ( booking_supplement_id = '1'  action_code = zjs_if_flight_legacy=>action_code-update  price = abap_true ) )
*                           IMPORTING et_messages            = DATA(lt_messages) ).
*    cl_abap_unit_assert=>assert_equals( act = lines( lt_messages )  exp = 1 ).
*    cl_abap_unit_assert=>assert_true( xsdbool( lt_messages[ 1 ] IS INSTANCE OF zjs_cx_flight_legacy ) ).
*    DATA lx TYPE REF TO zjs_cx_flight_legacy.
...
*                                     it_booking_supplementx = VALUE #( ( booking_id = mv_booking_id_unknown  booking_supplement_id = '1'  action_code = zjs_if_flight_legacy=>action_code-delete ) )
*                           IMPORTING et_messages            = DATA(lt_messages) ).
*    cl_abap_unit_assert=>assert_equals( act = lines( lt_messages )  exp = 1 ).
*    cl_abap_unit_assert=>assert_true( xsdbool( lt_messages[ 1 ] IS INSTANCE OF zjs_cx_flight_legacy ) ).
*    DATA lx TYPE REF TO zjs_cx_flight_legacy.
...
*                                     is_travelx             = VALUE #( travel_id = mv_travel_id )
*                                     it_booking_supplement  = VALUE #( ( booking_supplement_id = '1' ) )
*                                     it_booking_supplementx = VALUE #( ( booking_supplement_id = '1'  action_code = zjs_if_flight_legacy=>action_code-delete ) )
*                           IMPORTING et_messages            = DATA(lt_messages) ).
*    cl_abap_unit_assert=>assert_equals( act = lines( lt_messages )  exp = 1 ).
*    cl_abap_unit_assert=>assert_true( xsdbool( lt_messages[ 1 ] IS INSTANCE OF zjs_cx_flight_legacy ) ).
*    DATA lx TYPE REF TO zjs_cx_flight_legacy.
...
*                                     it_booking_supplementx = VALUE #( ( booking_id = mv_booking_id  booking_supplement_id = '10'  action_code = zjs_if_flight_legacy=>action_code-update  price = abap_true ) )
*                           IMPORTING et_messages            = DATA(lt_messages) ).
*    cl_abap_unit_assert=>assert_equals( act = lines( lt_messages )  exp = 1 ).
*    cl_abap_unit_assert=>assert_true( xsdbool( lt_messages[ 1 ] IS INSTANCE OF zjs_cx_flight_legacy ) ).
*    DATA lx TYPE REF TO zjs_cx_flight_legacy.
*    lx ?= lt_messages[ 1 ].
*    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-msgid  exp = mc_msgid ).
*    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-msgno  exp = zjs_cx_flight_legacy=>booking_supplement_unknown-msgno ).
*    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-attr1  exp = 'MV_TRAVEL_ID' ).
*    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-attr2  exp = 'MV_BOOKING_ID' ).
...
*                                     it_booking_supplementx = VALUE #( ( booking_id = mv_booking_id  action_code = zjs_if_flight_legacy=>action_code-update  price = abap_true ) )
*                           IMPORTING et_messages            = DATA(lt_messages) ).
*    cl_abap_unit_assert=>assert_equals( act = lines( lt_messages )  exp = 1 ).
*    cl_abap_unit_assert=>assert_true( xsdbool( lt_messages[ 1 ] IS INSTANCE OF zjs_cx_flight_legacy ) ).
*    DATA lx TYPE REF TO zjs_cx_flight_legacy.
*    lx ?= lt_messages[ 1 ].
*    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-msgid  exp = mc_msgid ).
*    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-msgno  exp = zjs_cx_flight_legacy=>booking_supplement_no_key-msgno ).
*    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-attr1  exp = 'MV_TRAVEL_ID' ).
*    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-attr2  exp = 'MV_BOOKING_ID' ).
*    cl_abap_unit_assert=>assert_equals( act = lx->mv_travel_id                   exp = mv_travel_id ).
*    cl_abap_unit_assert=>assert_equals( act = lx->mv_booking_id                  exp = mv_booking_id ).
...
*                                     it_booking_supplementx = VALUE #( ( booking_id = mv_booking_id  action_code = zjs_if_flight_legacy=>action_code-delete ) )
*                           IMPORTING et_messages            = DATA(lt_messages) ).
*    cl_abap_unit_assert=>assert_equals( act = lines( lt_messages )  exp = 1 ).
*    cl_abap_unit_assert=>assert_true( xsdbool( lt_messages[ 1 ] IS INSTANCE OF zjs_cx_flight_legacy ) ).
*    DATA lx TYPE REF TO zjs_cx_flight_legacy.
...
*    CLEAR lv_db_exists.
*    SELECT SINGLE FROM zjs_book_suppl FIELDS @abap_true WHERE travel_id             = @ls_booking_supplement-travel_id
...
*                                     it_bookingx = VALUE #( ( booking_id = mv_booking_id  action_code = zjs_if_flight_legacy=>action_code-delete ) )
*                           IMPORTING et_messages = DATA(lt_messages) ).
*    cl_abap_unit_assert=>assert_initial( lt_messages ).
*    gr_cut->update_travel( EXPORTING is_travel              = VALUE #( travel_id = mv_travel_id )
*                                     is_travelx             = VALUE #( travel_id = mv_travel_id )
...
*    CLEAR lv_db_exists.
*    SELECT SINGLE FROM zjs_book_suppl FIELDS @abap_true WHERE travel_id = @mv_travel_id AND booking_id = @mv_booking_id AND booking_supplement_id = '22' INTO @lv_db_exists.
*    cl_abap_unit_assert=>assert_true( lv_db_exists ).
*
...
*
*  METHOD dd_single.
*    CONSTANTS lc_booking_supplement_id TYPE zjs_de_booking_supplement_id VALUE '34'.
*    DATA lv_db_exists TYPE abap_bool.
*
...
*    " Check existence of Booking Supplement
*    CLEAR lv_db_exists.
*    SELECT SINGLE FROM zjs_book_suppl FIELDS @abap_true WHERE travel_id = @mv_travel_id AND booking_id = @mv_booking_id AND booking_supplement_id = @lc_booking_supplement_id INTO @lv_db_exists.
*    cl_abap_unit_assert=>assert_true( lv_db_exists ).
*
...
*    lx ?= lt_messages[ 1 ].
*    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-msgid  exp = mc_msgid ).
*    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-msgno  exp = zjs_cx_flight_legacy=>supplement_unknown-msgno ).
...
*    CLEAR lv_db_exists.
*    SELECT SINGLE FROM zjs_book_suppl FIELDS @abap_true WHERE travel_id  = @mv_travel_id AND booking_id = @mv_booking_id INTO @lv_db_exists.
*    cl_abap_unit_assert=>assert_false( lv_db_exists ).
*  ENDMETHOD.
*
*
...
*    lx ?= lt_messages[ 1 ].
*    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-msgid  exp = mc_msgid ).
*    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-msgno  exp = zjs_cx_flight_legacy=>booking_supplement_exists-msgno ).
*    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-attr1  exp = 'MV_TRAVEL_ID' ).
*    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-attr2  exp = 'MV_BOOKING_ID' ).
*    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-attr3  exp = 'MV_BOOKING_SUPPLEMENT_ID' ).
*    cl_abap_unit_assert=>assert_equals( act = lx->mv_travel_id                   exp = mv_travel_id ).
...
*    CONSTANTS lc_booking_id_1 TYPE zjs_de_booking_id VALUE '20'.
*    CONSTANTS lc_booking_id_2 TYPE zjs_de_booking_id VALUE '21'.
*    CONSTANTS lc_booking_supplement_id_1_1 TYPE zjs_de_booking_supplement_id VALUE '10'.
*    CONSTANTS lc_booking_supplement_id_2_1 TYPE zjs_de_booking_supplement_id VALUE '10'.
*    CONSTANTS lc_booking_supplement_id_2_2 TYPE zjs_de_booking_supplement_id VALUE '20'.
...
*    zjs_cl_flight_amdp=>convert_currency(
*      EXPORTING
*        iv_amount               = '30.00'
...
*    DATA ls_travel_sel TYPE zjs_travel.
*    SELECT SINGLE agency_id, customer_id, total_price, currency_code, description FROM zjs_travel WHERE travel_id = @ls_travel-travel_id INTO CORRESPONDING FIELDS OF @ls_travel_sel.
*    cl_abap_unit_assert=>assert_subrc( ).
...
*    SELECT COUNT( * ) FROM zjs_booking WHERE travel_id = @ls_travel-travel_id INTO @DATA(lv_count).
*    cl_abap_unit_assert=>assert_equals( act = lv_count  exp = 2 ).
*    DATA ls_booking_sel TYPE zjs_booking.
*    SELECT SINGLE customer_id, flight_date FROM zjs_booking WHERE travel_id  = @ls_travel-travel_id
...
*    cl_abap_unit_assert=>assert_equals( act = lv_count  exp = 3 ).
*    DATA ls_book_suppl_sel TYPE zjs_book_suppl.
*    SELECT SINGLE price, currency_code FROM zjs_book_suppl WHERE travel_id             = @ls_travel-travel_id
*                                                              AND booking_id            = @lc_booking_id_2
...
*    CONSTANTS lc_booking_id_1 TYPE zjs_de_booking_id VALUE '20'.
*    CONSTANTS lc_booking_supplement_id_1_1 TYPE zjs_de_booking_supplement_id VALUE '10'.
*    CONSTANTS lc_booking_supplement_id_1_2 TYPE zjs_de_booking_supplement_id VALUE '20'.
*    CONSTANTS lc_booking_supplement_id_1_3 TYPE zjs_de_booking_supplement_id VALUE '30'.
*    gr_cut->create_travel(
*      EXPORTING
*        is_travel             = VALUE #( agency_id     = gv_agency_id_1
*                                         customer_id   = gv_customer_id_1
...
*
*    CLEAR lv_db_exists.
*    SELECT SINGLE FROM zjs_book_suppl FIELDS @abap_true WHERE travel_id             = @ls_travel-travel_id
...
*    CONSTANTS lc_booking_id_2 TYPE zjs_de_booking_id VALUE '10'.
*    CONSTANTS lc_booking_supplement_id_2_1 TYPE zjs_de_booking_supplement_id VALUE '10'.
*    CONSTANTS lc_booking_supplement_id_1_4 TYPE zjs_de_booking_supplement_id VALUE '40'.
*    CONSTANTS lc_diff TYPE zjs_de_supplement_price VALUE '123.00'.
*    DATA lv_new_price TYPE zjs_de_supplement_price.
...
*                                            flight_price  = '200.00'
*                                            currency_code = 'EUR' ) )
*        it_bookingx            = VALUE #( ( booking_id = lc_booking_id_1  action_code = zjs_if_flight_legacy=>action_code-update  customer_id = abap_true )
*                                          ( booking_id = lc_booking_id_2  action_code = zjs_if_flight_legacy=>action_code-create ) )
*        it_booking_supplement  = VALUE #( ( booking_id            = lc_booking_id_1
*                                            booking_supplement_id = lc_booking_supplement_id_1_1
*                                            price                 = lv_new_price
*                                            currency_code         = 'EUR' )
...
*    " Check Travel
*    DATA ls_travel_sel TYPE zjs_travel.
*    SELECT SINGLE total_price, currency_code, description FROM zjs_travel WHERE travel_id = @ls_travel-travel_id INTO CORRESPONDING FIELDS OF @ls_travel_sel.
*    cl_abap_unit_assert=>assert_subrc( ).
...
*    " -- Updated Booking
*    DATA ls_booking_sel TYPE zjs_booking.
*    SELECT SINGLE booking_date, customer_id FROM zjs_booking WHERE travel_id = @ls_travel-travel_id AND booking_id = @lc_booking_id_1 INTO CORRESPONDING FIELDS OF @ls_booking_sel.
*    cl_abap_unit_assert=>assert_subrc( ).
...
*    " -- New Booking
*    CLEAR ls_booking_sel.
*    SELECT SINGLE customer_id, flight_date FROM zjs_booking WHERE travel_id = @ls_travel-travel_id AND booking_id = @lc_booking_id_2 INTO CORRESPONDING FIELDS OF @ls_booking_sel.
*    cl_abap_unit_assert=>assert_subrc( ).
...
*    DATA ls_booking_supplement_sel TYPE zjs_book_suppl.
*    SELECT SINGLE supplement_id, price FROM zjs_book_suppl WHERE travel_id             = @ls_travel-travel_id
*                                                              AND booking_id            = @lc_booking_id_1
...
*    SELECT SINGLE supplement_id FROM zjs_book_suppl WHERE travel_id             = @ls_travel-travel_id
*                                                       AND booking_id            = @lc_booking_id_1
...
*                                     is_travelx  = VALUE #( travel_id = ls_travel-travel_id )
*                                     it_booking  = VALUE #( ( booking_id = lc_booking_id_1 ) )
*                                     it_bookingx = VALUE #( ( booking_id = lc_booking_id_1  action_code = zjs_if_flight_legacy=>action_code-delete ) )
*                           IMPORTING et_messages = lt_messages ).
*    cl_abap_unit_assert=>assert_initial( lt_messages ).
*
*    gr_cut->get_travel( EXPORTING iv_travel_id          = ls_travel-travel_id
...
*    lx ?= lt_messages[ 1 ].
*    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-msgid  exp = mc_msgid ).
*    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-msgno  exp = zjs_cx_flight_legacy=>booking_supplement_suppl_id_u-msgno ).
*    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-attr1  exp = 'MV_TRAVEL_ID' ).
*    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-attr2  exp = 'MV_BOOKING_ID' ).
*    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-attr3  exp = 'MV_BOOKING_SUPPLEMENT_ID' ).
*    cl_abap_unit_assert=>assert_equals( act = lx->mv_travel_id                   exp = ls_booking_supplement-travel_id ).
...
*    SELECT SINGLE supplement_id, price FROM zjs_book_suppl WHERE travel_id             = @ls_booking_supplement-travel_id
*                                                              AND booking_id            = @ls_booking_supplement-booking_id
*                                                              AND booking_supplement_id = @ls_booking_supplement-booking_supplement_id
...
*
*    " Provide price and currency
*    CONSTANTS lc_diff TYPE zjs_de_supplement_price VALUE '9.99'.
*    DATA lv_price         TYPE zjs_de_supplement_price.
*    DATA lv_currency_code TYPE zjs_de_currency_code.
...
*
*    DATA ls_booking_supplement_sel TYPE zjs_book_suppl.
*    SELECT SINGLE price, currency_code FROM zjs_book_suppl WHERE travel_id = @mv_travel_id AND booking_id = @mv_booking_id AND booking_supplement_id = '10' INTO CORRESPONDING FIELDS OF @ls_booking_supplement_sel.
*    cl_abap_unit_assert=>assert_subrc( ).
...
*
*    CLEAR ls_booking_supplement_sel.
*    SELECT SINGLE price, currency_code FROM zjs_book_suppl WHERE travel_id = @mv_travel_id AND booking_id = @mv_booking_id AND booking_supplement_id = '20' INTO CORRESPONDING FIELDS OF @ls_booking_supplement_sel.
*    cl_abap_unit_assert=>assert_subrc( ).
...
*    lx ?= lt_messages[ 1 ].
*    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-msgid  exp = mc_msgid ).
*    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-msgno  exp = zjs_cx_flight_legacy=>booking_supplement_pri_curr_u-msgno ).
*    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-attr1  exp = 'MV_TRAVEL_ID' ).
*    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-attr2  exp = 'MV_BOOKING_ID' ).
*    cl_abap_unit_assert=>assert_equals( act = lx->if_t100_message~t100key-attr3  exp = 'MV_BOOKING_SUPPLEMENT_ID' ).
*    cl_abap_unit_assert=>assert_equals( act = lx->mv_travel_id                   exp = ls_booking_supplement-travel_id ).
...
*    CONSTANTS cv_travel_id_final_1       TYPE zjs_de_travel_id VALUE '42'.
*    CONSTANTS cv_travel_id_final_2       TYPE zjs_de_travel_id VALUE '314'.
*    CONSTANTS cv_booking_id_final_1      TYPE zjs_de_booking_id VALUE '61'.
*    CONSTANTS cv_booking_id_final_2      TYPE zjs_de_booking_id VALUE '62'.
*    CONSTANTS cv_booksuppl_id_final_1    TYPE zjs_de_booking_supplement_id VALUE '1'.
*    CONSTANTS cv_booksuppl_id_final_2    TYPE zjs_de_booking_supplement_id VALUE '2'.
*
*    DATA lt_mapping TYPE zjs_if_flight_legacy=>tt_ln_bookingsuppl_mapping.
*
...
*                ]-final
*        exp = VALUE zjs_if_flight_legacy=>ts_ln_bookingsuppl(
*                  travel_id             = cv_travel_id_final_1
*                  booking_id            = cv_booking_id_final_1
