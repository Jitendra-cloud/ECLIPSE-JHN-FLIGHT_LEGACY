CLASS zjs_cl_flight_data_generator DEFINITION
  PUBLIC
  FINAL
  CREATE PUBLIC .
  ...
  PRIVATE SECTION.
    CLASS-METHODS:
      ...
      "!  <br/>
      "! Propagation to zjs_cl_flight_legacy=>calculate_flight_price.<br/>
      ...
      calculate_flight_price
        ...
        RETURNING
          VALUE(rv_price)           TYPE zjs_de_flight_price.

ENDCLASS.



CLASS zjs_cl_flight_data_generator IMPLEMENTATION.


  METHOD calculate_flight_price.
    rv_price = zjs_cl_flight_legacy=>calculate_flight_price(
  ...
  METHOD if_oo_adt_classrun~main.
    ...
    out->write( 'Generate Data: Airport      ZJS_AIRPORT' ) ##NO_TEXT.
    lcl_airport_data_generator=>lif_data_generator~create( out ).

    out->write( 'Generate Data: Carrier      ZJS_CARRIER' ) ##NO_TEXT.
    lcl_carrier_data_generator=>lif_data_generator~create( out ).
    out->write( 'Generate Data: Connection   ZJS_CONNECTION' ) ##NO_TEXT.
    lcl_connection_data_generator=>lif_data_generator~create( out ).
    out->write( 'Generate Data: Flight       ZJS_FLIGHT' ) ##NO_TEXT.
    lcl_flight_data_generator=>lif_data_generator~create( out ).


    out->write( 'Generate Data: Agency       ZJS_AGENCY' ) ##NO_TEXT.
    lcl_agency_data_generator=>lif_data_generator~create( out ).

    out->write( 'Generate Data: Customer      ZJS_CUSTOMER' ) ##NO_TEXT.
    lcl_customer_data_generator=>lif_data_generator~create( out ).

    out->write( 'Generate Data: Supplement      ZJS_SUPPLEMENT' ) ##NO_TEXT.
    lcl_supplement_data_generator=>lif_data_generator~create( out ).

    out->write( 'Generate Data: Travel      ZJS_TRAVEL' ) ##NO_TEXT.
    out->write( 'Generate Data: Booking      ZJS_BOOKING' ) ##NO_TEXT.
    out->write( 'Generate Data: Booking Supplement      ZJS_BOOK_SUPPL' ) ##NO_TEXT.
    ...
    DATA lo_badi TYPE REF TO zjs_data_generation_badi.
    GET BADI lo_badi.
...
""Added seprately for new field extension
    SELECT * FROM /dmo/agency INTO TABLE @DATA(lt_agency).
    LOOP AT lt_agency ASSIGNING FIELD-SYMBOL(<fs_agency>).
      UPDATE zjs_agency SET zzsloganzjs = @<fs_agency>-/dmo/zzsloganzag WHERE agency_id = @<fs_agency>-agency_id.
    ENDLOOP.
""Added seprately for new field extension
