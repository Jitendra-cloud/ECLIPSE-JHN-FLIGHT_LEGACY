CLASS lcl_common_checks DEFINITION FINAL CREATE PRIVATE.
  PUBLIC SECTION.
    CLASS-METHODS is_currency_code_valid IMPORTING iv_currency_code   TYPE zjs_de_currency_code
                                         CHANGING  ct_messages        TYPE zjs_if_flight_legacy=>tt_if_t100_message
                                         RETURNING VALUE(rv_is_valid) TYPE abap_boolean.
    CLASS-METHODS is_customer_id_valid IMPORTING iv_customer_id     TYPE zjs_de_customer_id
                                       CHANGING  ct_messages        TYPE zjs_if_flight_legacy=>tt_if_t100_message
                                       RETURNING VALUE(rv_is_valid) TYPE abap_boolean.
  PRIVATE SECTION.
    CLASS-DATA mt_currency_code TYPE SORTED TABLE OF zjs_de_currency_code WITH UNIQUE KEY table_line.
    CLASS-DATA mt_customer_id TYPE SORTED TABLE OF zjs_de_customer_id WITH UNIQUE KEY table_line.
ENDCLASS.
...
CLASS lcl_common_checks IMPLEMENTATION.
  METHOD is_currency_code_valid.
    ...
    ELSE.
      APPEND NEW zjs_cx_flight_legacy( textid = zjs_cx_flight_legacy=>currency_unknown  currency_code = iv_currency_code ) TO ct_messages.
    ENDIF.
  ENDMETHOD.
  ...
  METHOD is_customer_id_valid.
    ...
    IF mt_customer_id IS INITIAL.
      ...
      SELECT DISTINCT customer_id FROM zjs_customer INTO TABLE @mt_customer_id. "#EC CI_NOWHERE
    ENDIF.
    READ TABLE mt_customer_id TRANSPORTING NO FIELDS WITH TABLE KEY table_line = iv_customer_id.
    IF sy-subrc = 0.
      rv_is_valid = abap_true.
    ELSE.
      APPEND NEW zjs_cx_flight_legacy( textid = zjs_cx_flight_legacy=>customer_unkown  customer_id = iv_customer_id ) TO ct_messages.
    ENDIF.
  ENDMETHOD.
ENDCLASS.
...
CLASS lcl_booking_supplement_buffer DEFINITION FINAL CREATE PRIVATE. "FRIENDS ltc_booking_supplement.
  PUBLIC SECTION.
    ...
    METHODS adjust_numbers IMPORTING it_booking_mapping             TYPE zjs_if_flight_legacy=>tt_ln_booking_mapping
                           RETURNING VALUE(rt_bookingsuppl_mapping) TYPE zjs_if_flight_legacy=>tt_ln_bookingsuppl_mapping.
    METHODS save.
    METHODS initialize.
    ...
    METHODS cud_prep IMPORTING it_booking_supplement  TYPE zjs_tt_booking_supplement
                               it_booking_supplementx TYPE zjs_tt_booking_supplementx
                               iv_no_delete_check     TYPE abap_bool OPTIONAL
                     EXPORTING et_booking_supplement  TYPE zjs_tt_booking_supplement
                               et_messages            TYPE zjs_if_flight_legacy=>tt_if_t100_message.
    ...
    METHODS get IMPORTING it_booking_supplement  TYPE zjs_tt_booking_supplement
  ...
  PRIVATE SECTION.
    ...
    " Main buffer
    DATA: mt_create_buffer TYPE zjs_tt_booking_supplement,
          mt_update_buffer TYPE zjs_tt_booking_supplement,
          mt_delete_buffer TYPE zjs_tt_booking_supplement_key.
    " Temporary buffer valid during create / update / delete Travel
    DATA: mt_create_buffer_2 TYPE zjs_tt_booking_supplement,
          mt_update_buffer_2 TYPE zjs_tt_booking_supplement,
          mt_delete_buffer_2 TYPE zjs_tt_booking_supplement_key.

    DATA mt_supplement TYPE SORTED TABLE OF zjs_supplement WITH UNIQUE KEY supplement_id.

    METHODS _create IMPORTING it_booking_supplement TYPE zjs_tt_booking_supplement
                    EXPORTING et_booking_supplement TYPE zjs_tt_booking_supplement
                              et_messages           TYPE zjs_if_flight_legacy=>tt_if_t100_message.
    METHODS _update IMPORTING it_booking_supplement  TYPE zjs_tt_booking_supplement
                              it_booking_supplementx TYPE zjs_tt_booking_supplementx
                    EXPORTING et_booking_supplement  TYPE zjs_tt_booking_supplement
                              et_messages            TYPE zjs_if_flight_legacy=>tt_if_t100_message.
    METHODS _delete IMPORTING it_booking_supplement TYPE zjs_tt_booking_supplement
                              iv_no_delete_check    TYPE abap_bool
                    EXPORTING et_messages           TYPE zjs_if_flight_legacy=>tt_if_t100_message.

    METHODS _check IMPORTING is_booking_supplement  TYPE zjs_book_suppl
                             is_booking_supplementx TYPE zjs_st_booking_supplementx OPTIONAL
                             iv_change_mode         TYPE zjs_cl_flight_legacy=>ty_change_mode
                   CHANGING  ct_messages            TYPE zjs_if_flight_legacy=>tt_if_t100_message
                   RETURNING VALUE(rv_is_valid)     TYPE abap_bool.
    METHODS _check_supplement IMPORTING is_booking_supplement  TYPE zjs_book_suppl
                                        is_booking_supplementx TYPE zjs_st_booking_supplementx OPTIONAL
                                        iv_change_mode         TYPE zjs_cl_flight_legacy=>ty_change_mode
                              CHANGING  ct_messages            TYPE zjs_if_flight_legacy=>tt_if_t100_message
                              RETURNING VALUE(rv_is_valid)     TYPE abap_bool.
    METHODS _check_currency_code IMPORTING is_booking_supplement  TYPE zjs_book_suppl
                                           is_booking_supplementx TYPE zjs_st_booking_supplementx OPTIONAL
                                           iv_change_mode         TYPE zjs_cl_flight_legacy=>ty_change_mode
                                 CHANGING  ct_messages            TYPE zjs_if_flight_legacy=>tt_if_t100_message
                                 RETURNING VALUE(rv_is_valid)     TYPE abap_bool.
    METHODS _determine IMPORTING iv_change_mode        TYPE zjs_cl_flight_legacy=>ty_change_mode
                       CHANGING  cs_booking_supplement TYPE zjs_book_suppl.
ENDCLASS.
...
CLASS lcl_booking_supplement_buffer IMPLEMENTATION.
  ...
  METHOD _create.
    ...
    TYPES: BEGIN OF ts_travel_booking_suppl_id,
             travel_id             TYPE zjs_de_travel_id,
             booking_id            TYPE zjs_de_booking_id,
             booking_supplement_id TYPE zjs_de_booking_supplement_id,
    ...
    SELECT FROM zjs_book_suppl FIELDS travel_id, booking_id, booking_supplement_id
    ...
    LOOP AT it_booking_supplement INTO DATA(ls_booking_supplement_create) ##INTO_OK.
      ...
      IF ls_booking_supplement_create-booking_supplement_id IS INITIAL.
        APPEND NEW zjs_cx_flight_legacy( textid     = zjs_cx_flight_legacy=>booking_supplement_no_key
      ...
      IF sy-subrc = 0.
        APPEND NEW zjs_cx_flight_legacy( textid     = zjs_cx_flight_legacy=>booking_supplement_exists
                                          travel_id  = ls_booking_supplement_create-travel_id
                                          booking_id = ls_booking_supplement_create-booking_id
      ...
      IF _check( EXPORTING is_booking_supplement = ls_booking_supplement_create
                           iv_change_mode        = zjs_cl_flight_legacy=>change_mode-create
                 CHANGING  ct_messages           = et_messages ) = abap_false.
        RETURN.
      ENDIF.

      " standard determinations
      _determine( EXPORTING iv_change_mode        = zjs_cl_flight_legacy=>change_mode-create
  ...
  METHOD _update.
    ...
    LOOP AT it_booking_supplement ASSIGNING FIELD-SYMBOL(<s_booking_supplement_update>) WHERE booking_supplement_id = 0. "#EC CI_SORTSEQ
      APPEND NEW zjs_cx_flight_legacy( textid = zjs_cx_flight_legacy=>booking_supplement_no_key  travel_id = <s_booking_supplement_update>-travel_id  booking_id = <s_booking_supplement_update>-booking_id ) TO et_messages.
      RETURN.
    ENDLOOP.

    DATA lt_book_suppl TYPE SORTED TABLE OF zjs_book_suppl WITH UNIQUE KEY travel_id  booking_id  booking_supplement_id.
    SELECT * FROM zjs_book_suppl FOR ALL ENTRIES IN @it_booking_supplement WHERE travel_id             = @it_booking_supplement-travel_id
    ...
    FIELD-SYMBOLS <s_buffer_booking_supplement> TYPE zjs_book_suppl.
    DATA ls_buffer_booking_supplement TYPE zjs_book_suppl.
    LOOP AT it_booking_supplement ASSIGNING <s_booking_supplement_update>.
      ...
      IF sy-subrc = 0." Error: Record to be updated marked for deletion
        APPEND NEW zjs_cx_flight_legacy( textid                = zjs_cx_flight_legacy=>booking_supplement_unknown
      ...
      IF sy-subrc <> 0.
        APPEND NEW zjs_cx_flight_legacy( textid                = zjs_cx_flight_legacy=>booking_supplement_no_control
                                          travel_id             = <s_booking_supplement_update>-travel_id
                                          booking_id            = <s_booking_supplement_update>-booking_id
                                          booking_supplement_id = <s_booking_supplement_update>-booking_supplement_id ) TO et_messages.
        RETURN.
      ...
      IF <s_booking_supplementx>-supplement_id = abap_true AND <s_booking_supplement_update>-supplement_id <> <s_buffer_booking_supplement>-supplement_id.
        ...
        APPEND NEW zjs_cx_flight_legacy( textid                = zjs_cx_flight_legacy=>booking_supplement_suppl_id_u
                                          travel_id             = <s_booking_supplement_update>-travel_id
                                          booking_id            = <s_booking_supplement_update>-booking_id
                                          booking_supplement_id = <s_booking_supplement_update>-booking_supplement_id ) TO et_messages.
        RETURN.
      ...
      IF   ( <s_booking_supplementx>-price = abap_true  AND <s_booking_supplementx>-currency_code = abap_false )
        ...
        APPEND NEW zjs_cx_flight_legacy( textid                = zjs_cx_flight_legacy=>booking_supplement_pri_curr_u
                                          travel_id             = <s_booking_supplement_update>-travel_id
                                          booking_id            = <s_booking_supplement_update>-booking_id
                                          booking_supplement_id = <s_booking_supplement_update>-booking_supplement_id ) TO et_messages.
        RETURN.
      ...
      IF _check( EXPORTING is_booking_supplement  = <s_buffer_booking_supplement>
                           ...
                           iv_change_mode         = zjs_cl_flight_legacy=>change_mode-update
                 CHANGING  ct_messages            = et_messages ) = abap_false.
        RETURN.
      ENDIF.
      ...
      _determine( EXPORTING iv_change_mode        = zjs_cl_flight_legacy=>change_mode-update
  ...
  METHOD _delete.
    ...
    IF iv_no_delete_check = abap_false.
      DATA lt_book_suppl_db TYPE zjs_tt_booking_supplement_key.
      SELECT travel_id, booking_id, booking_supplement_id FROM zjs_book_suppl FOR ALL ENTRIES IN @lt_booking_supplement
  ...
  METHOD adjust_numbers.
    DATA: lt_create_buffer TYPE zjs_tt_booking_supplement.
  ...
  METHOD save.
    ...
    IF mt_create_buffer IS NOT INITIAL.
      ASSERT mt_create_buffer[ lines( mt_create_buffer ) ]-travel_id < zjs_if_flight_legacy=>late_numbering_boundary.
    ENDIF.

    INSERT zjs_book_suppl FROM TABLE @mt_create_buffer.
    UPDATE zjs_book_suppl FROM TABLE @mt_update_buffer.
    DELETE zjs_book_suppl FROM TABLE @( CORRESPONDING #( mt_delete_buffer ) ).
  ENDMETHOD.
  ...
  METHOD cud_prep.
    ...
    DATA lt_booking_supplement_c  TYPE zjs_tt_booking_supplement.
    DATA lt_booking_supplement_u  TYPE zjs_tt_booking_supplement.
    DATA lt_booking_supplement_d  TYPE zjs_tt_booking_supplement.
    DATA lt_booking_supplementx_u TYPE zjs_tt_booking_supplementx.
    LOOP AT it_booking_supplement ASSIGNING FIELD-SYMBOL(<s_booking_supplement>).
      ...
      IF sy-subrc <> 0.
        ...
        RETURN.
      ENDIF.
      CASE CONV zjs_if_flight_legacy=>action_code_enum( <s_booking_supplementx>-action_code ).
        WHEN zjs_if_flight_legacy=>action_code-create.
          INSERT <s_booking_supplement>  INTO TABLE lt_booking_supplement_c.
        WHEN zjs_if_flight_legacy=>action_code-update.
        ...
        WHEN zjs_if_flight_legacy=>action_code-delete.
  ...
  METHOD _check_supplement.
    rv_is_valid = abap_true.
    IF iv_change_mode = zjs_cl_flight_legacy=>change_mode-create OR ( iv_change_mode = zjs_cl_flight_legacy=>change_mode-update AND is_booking_supplementx-supplement_id = abap_true ).
      IF mt_supplement IS INITIAL.
        SELECT * FROM zjs_supplement INTO TABLE @mt_supplement. "#EC CI_NOWHERE
      ENDIF.
      READ TABLE mt_supplement TRANSPORTING NO FIELDS WITH TABLE KEY supplement_id = is_booking_supplement-supplement_id.
      IF sy-subrc <> 0.
        rv_is_valid = abap_false.
        APPEND NEW zjs_cx_flight_legacy( textid = zjs_cx_flight_legacy=>supplement_unknown  supplement_id = is_booking_supplement-supplement_id ) TO ct_messages.
      ENDIF.
    ENDIF.
  ENDMETHOD.
  ...
  METHOD _check_currency_code.
    rv_is_valid = abap_true.
    IF   ( iv_change_mode = zjs_cl_flight_legacy=>change_mode-create AND is_booking_supplement-currency_code IS NOT INITIAL ) " Will be derived if initial
      OR ( iv_change_mode = zjs_cl_flight_legacy=>change_mode-update AND is_booking_supplementx-currency_code = abap_true ).
  ...
  METHOD _determine.
    IF iv_change_mode = zjs_cl_flight_legacy=>change_mode-create.
...
CLASS lcl_booking_buffer DEFINITION FINAL CREATE PRIVATE. "FRIENDS ltc_booking.
  PUBLIC SECTION.
    ...
    METHODS adjust_numbers IMPORTING it_travel_mapping         TYPE zjs_if_flight_legacy=>tt_ln_travel_mapping
                           RETURNING VALUE(rt_booking_mapping) TYPE zjs_if_flight_legacy=>tt_ln_booking_mapping.
    METHODS save.
    METHODS initialize.
    METHODS check_booking_id IMPORTING iv_travel_id       TYPE zjs_de_travel_id
    ...
    "! @parameter iv_no_delete_check | In some cases we do not need to check the existence of a record to be deleted, as this check has been done before.
    "!                               | E.g. delete all subnodes of a node to be deleted.  In this case we have read the subnodes to get their keys.
    METHODS cud_prep IMPORTING it_booking         TYPE zjs_tt_booking
                               it_bookingx        TYPE zjs_tt_bookingx
                               iv_no_delete_check TYPE abap_bool OPTIONAL
                     EXPORTING et_booking         TYPE zjs_tt_booking
                               et_messages        TYPE zjs_if_flight_legacy=>tt_if_t100_message.
    "! Add content of the temporary buffer to the real buffer and clear the temporary buffer
    METHODS cud_copy.
    "! Discard content of the temporary buffer
    ...
    METHODS get IMPORTING it_booking             TYPE zjs_tt_booking
                          iv_include_buffer      TYPE abap_boolean
                          iv_include_temp_buffer TYPE abap_boolean
                EXPORTING et_booking             TYPE zjs_tt_booking.
  ...
  PRIVATE SECTION.
    ...
    " Main buffer
    DATA: mt_create_buffer TYPE zjs_tt_booking,
          mt_update_buffer TYPE zjs_tt_booking,
          mt_delete_buffer TYPE zjs_tt_booking_key.
    " Temporary buffer valid during create / update / delete Travel
    DATA: mt_create_buffer_2 TYPE zjs_tt_booking,
          mt_update_buffer_2 TYPE zjs_tt_booking,
          mt_delete_buffer_2 TYPE zjs_tt_booking_key.
    ...
    METHODS _create IMPORTING it_booking  TYPE zjs_tt_booking
                    EXPORTING et_booking  TYPE zjs_tt_booking
                              et_messages TYPE zjs_if_flight_legacy=>tt_if_t100_message.
    METHODS _update IMPORTING it_booking  TYPE zjs_tt_booking
                              it_bookingx TYPE zjs_tt_bookingx
                    EXPORTING et_booking  TYPE zjs_tt_booking
                              et_messages TYPE zjs_if_flight_legacy=>tt_if_t100_message.
    METHODS _delete IMPORTING it_booking         TYPE zjs_tt_booking
                              iv_no_delete_check TYPE abap_bool
                    EXPORTING et_messages        TYPE zjs_if_flight_legacy=>tt_if_t100_message.

    METHODS _check IMPORTING is_booking         TYPE zjs_booking
                             is_bookingx        TYPE zjs_st_bookingx OPTIONAL
                             iv_change_mode     TYPE zjs_cl_flight_legacy=>ty_change_mode
                   CHANGING  ct_messages        TYPE zjs_if_flight_legacy=>tt_if_t100_message
                   RETURNING VALUE(rv_is_valid) TYPE abap_bool.
    METHODS _check_booking_date IMPORTING is_booking         TYPE zjs_booking
                                          is_bookingx        TYPE zjs_st_bookingx OPTIONAL
                                          iv_change_mode     TYPE zjs_cl_flight_legacy=>ty_change_mode
                                CHANGING  ct_messages        TYPE zjs_if_flight_legacy=>tt_if_t100_message
                                RETURNING VALUE(rv_is_valid) TYPE abap_bool.
    METHODS _check_customer IMPORTING is_booking         TYPE zjs_booking
                                      is_bookingx        TYPE zjs_st_bookingx OPTIONAL
                                      iv_change_mode     TYPE zjs_cl_flight_legacy=>ty_change_mode
                            CHANGING  ct_messages        TYPE zjs_if_flight_legacy=>tt_if_t100_message
                            RETURNING VALUE(rv_is_valid) TYPE abap_bool.
    METHODS _check_flight IMPORTING is_booking         TYPE zjs_booking
                                    is_bookingx        TYPE zjs_st_bookingx OPTIONAL
                                    iv_change_mode     TYPE zjs_cl_flight_legacy=>ty_change_mode
                          CHANGING  ct_messages        TYPE zjs_if_flight_legacy=>tt_if_t100_message
                          RETURNING VALUE(rv_is_valid) TYPE abap_bool.
    METHODS _check_currency_code IMPORTING is_booking         TYPE zjs_booking
                                           is_bookingx        TYPE zjs_st_bookingx OPTIONAL
                                           iv_change_mode     TYPE zjs_cl_flight_legacy=>ty_change_mode
                                 CHANGING  ct_messages        TYPE zjs_if_flight_legacy=>tt_if_t100_message
                                 RETURNING VALUE(rv_is_valid) TYPE abap_bool.
    METHODS _determine IMPORTING iv_change_mode TYPE zjs_cl_flight_legacy=>ty_change_mode
                       CHANGING  cs_booking     TYPE zjs_booking.
ENDCLASS.
...
CLASS lcl_booking_buffer IMPLEMENTATION.
  ...
  METHOD _create.
    ...
    SELECT FROM zjs_booking FIELDS travel_id, booking_id FOR ALL ENTRIES IN @it_booking WHERE travel_id = @it_booking-travel_id AND booking_id = @it_booking-booking_id INTO CORRESPONDING FIELDS OF TABLE @lt_travel_booking_id.
    ...
    LOOP AT it_booking INTO DATA(ls_booking_create) ##INTO_OK.
      ...
      IF ls_booking_create-booking_id IS INITIAL.
        APPEND NEW zjs_cx_flight_legacy( textid = zjs_cx_flight_legacy=>booking_no_key  travel_id = ls_booking_create-travel_id ) TO et_messages.
        RETURN.
      ENDIF.
      ...
      IF sy-subrc = 0.
        APPEND NEW zjs_cx_flight_legacy( textid = zjs_cx_flight_legacy=>booking_exists  travel_id = ls_booking_create-travel_id  booking_id = ls_booking_create-booking_id ) TO et_messages.
        RETURN.
      ENDIF.
  ...
  METHOD _update.
    ...
    LOOP AT it_booking ASSIGNING FIELD-SYMBOL(<s_booking_update>) WHERE booking_id = 0. "#EC CI_SORTSEQ
      APPEND NEW zjs_cx_flight_legacy( textid = zjs_cx_flight_legacy=>booking_no_key  travel_id = <s_booking_update>-travel_id ) TO et_messages.
      RETURN.
    ENDLOOP.

    DATA lt_booking TYPE SORTED TABLE OF zjs_booking WITH UNIQUE KEY travel_id  booking_id.
    SELECT * FROM zjs_booking FOR ALL ENTRIES IN @it_booking WHERE travel_id = @it_booking-travel_id AND booking_id = @it_booking-booking_id INTO TABLE @lt_booking.

    FIELD-SYMBOLS <s_buffer_booking> TYPE zjs_booking.
    DATA ls_buffer_booking TYPE zjs_booking.
    LOOP AT it_booking ASSIGNING <s_booking_update>.
      ...
      IF sy-subrc = 0." Error: Record to be updated marked for deletion
        APPEND NEW zjs_cx_flight_legacy( textid = zjs_cx_flight_legacy=>booking_unknown  travel_id = <s_booking_update>-travel_id  booking_id = <s_booking_update>-booking_id ) TO et_messages.
        RETURN.
      ENDIF.
      ...
      READ TABLE it_bookingx ASSIGNING FIELD-SYMBOL(<s_bookingx>) WITH KEY travel_id   = <s_booking_update>-travel_id
                                                                           ...
                                                                           action_code = zjs_if_flight_legacy=>action_code-update.
      IF sy-subrc <> 0.
        APPEND NEW zjs_cx_flight_legacy( textid     = zjs_cx_flight_legacy=>booking_no_control
      ...
      IF   ( <s_bookingx>-carrier_id    = abap_true AND ( <s_booking_update>-carrier_id    <> <s_buffer_booking>-carrier_id    ) )
        ...
        APPEND NEW zjs_cx_flight_legacy( textid     = zjs_cx_flight_legacy=>booking_flight_u
                                          travel_id  = <s_booking_update>-travel_id
                                          booking_id = <s_booking_update>-booking_id ) TO et_messages.
        RETURN.
      ENDIF.
      ...
      IF   ( <s_bookingx>-flight_price = abap_true  AND <s_bookingx>-currency_code = abap_false )
        ...
        " Price and currency code must be changed together
        APPEND NEW zjs_cx_flight_legacy( textid     = zjs_cx_flight_legacy=>booking_price_currency_u
                                          travel_id  = <s_booking_update>-travel_id
                                          booking_id = <s_booking_update>-booking_id ) TO et_messages.
        RETURN.
      ENDIF.
  ...
  METHOD _delete.
    ...
    LOOP AT it_booking ASSIGNING FIELD-SYMBOL(<s_booking_delete>) WHERE booking_id = 0. "#EC CI_SORTSEQ
      APPEND NEW zjs_cx_flight_legacy( textid = zjs_cx_flight_legacy=>booking_no_key  travel_id = <s_booking_delete>-travel_id ) TO et_messages.
      RETURN.
    ENDLOOP.
    ...
    IF iv_no_delete_check = abap_false.
      DATA lt_booking_db TYPE zjs_tt_booking_key.
      SELECT travel_id, booking_id FROM zjs_booking FOR ALL ENTRIES IN @lt_booking WHERE travel_id = @lt_booking-travel_id AND booking_id = @lt_booking-booking_id INTO CORRESPONDING FIELDS OF TABLE @lt_booking_db.
    ENDIF.
    ...
    LOOP AT lt_booking ASSIGNING <s_booking_delete>.
      IF iv_no_delete_check = abap_false.
        ...
        IF sy-subrc <> 0.
          APPEND NEW zjs_cx_flight_legacy( textid = zjs_cx_flight_legacy=>booking_unknown  travel_id = <s_booking_delete>-travel_id  booking_id = <s_booking_delete>-booking_id ) TO et_messages.
          RETURN.
        ENDIF.
      ENDIF.
      INSERT VALUE #( travel_id = <s_booking_delete>-travel_id  booking_id = <s_booking_delete>-booking_id ) INTO TABLE mt_delete_buffer_2.
  ...
  METHOD adjust_numbers.
    DATA: lt_create_buffer TYPE zjs_tt_booking.
  ...
  METHOD save.
    ...
    INSERT zjs_booking FROM TABLE @mt_create_buffer.
    UPDATE zjs_booking FROM TABLE @mt_update_buffer.
    DELETE zjs_booking FROM TABLE @( CORRESPONDING #( mt_delete_buffer ) ).
  ENDMETHOD.
  ...
  METHOD check_booking_id." Here we can safely assume that the Travel ID has already been checked!
    ...
    IF iv_booking_id IS INITIAL.
      APPEND NEW zjs_cx_flight_legacy( textid = zjs_cx_flight_legacy=>booking_no_key  travel_id = iv_travel_id ) TO ct_messages.
      RETURN.
    ENDIF.

    IF line_exists( mt_delete_buffer[ travel_id = iv_travel_id  booking_id = iv_booking_id ] ).
      APPEND NEW zjs_cx_flight_legacy( textid = zjs_cx_flight_legacy=>booking_unknown  travel_id = iv_travel_id  booking_id = iv_booking_id ) TO ct_messages.
      RETURN.
    ENDIF.
    ...
    SELECT SINGLE FROM zjs_booking FIELDS @abap_true WHERE travel_id = @iv_travel_id AND booking_id = @iv_booking_id INTO @DATA(lv_db_exists).
  ...
  METHOD cud_prep.
    ...
    DATA lt_booking_c  TYPE zjs_tt_booking.
    DATA lt_booking_u  TYPE zjs_tt_booking.
    DATA lt_booking_d  TYPE zjs_tt_booking.
    DATA lt_bookingx_u TYPE zjs_tt_bookingx.
    LOOP AT it_booking ASSIGNING FIELD-SYMBOL(<s_booking>).
      ...
      IF sy-subrc <> 0.
        ...
        RETURN.
      ENDIF.
      CASE CONV zjs_if_flight_legacy=>action_code_enum( <s_bookingx>-action_code ).
        WHEN zjs_if_flight_legacy=>action_code-create.
  ...
  METHOD get.
    ...
    SELECT * FROM zjs_booking FOR ALL ENTRIES IN @it_booking WHERE travel_id  = @it_booking-travel_id
  ...
  METHOD _check_booking_date.
    rv_is_valid = abap_true.
    IF iv_change_mode = zjs_cl_flight_legacy=>change_mode-create OR ( iv_change_mode = zjs_cl_flight_legacy=>change_mode-update AND is_bookingx-booking_date = abap_true ).

      " A. Booking Date must not be initial
      " B. When the record is created it must not be in the past
      IF is_booking-booking_date IS INITIAL OR is_booking-booking_date = '' OR ( iv_change_mode = zjs_cl_flight_legacy=>change_mode-create AND is_booking-booking_date < cl_abap_context_info=>get_system_date( ) ).
        rv_is_valid = abap_false.
        APPEND NEW zjs_cx_flight_legacy( textid = zjs_cx_flight_legacy=>booking_booking_date_invalid  travel_id = is_booking-travel_id  booking_id = is_booking-booking_id  booking_date = is_booking-booking_date ) TO ct_messages.
      ENDIF.
    ENDIF.
  ENDMETHOD.
  ...
  METHOD _check_customer.
    rv_is_valid = abap_true.
    IF iv_change_mode = zjs_cl_flight_legacy=>change_mode-create OR ( iv_change_mode = zjs_cl_flight_legacy=>change_mode-update AND is_bookingx-customer_id = abap_true ).
  ...
  METHOD _check_flight.
    rv_is_valid = abap_true.
    IF     iv_change_mode = zjs_cl_flight_legacy=>change_mode-create
      OR ( iv_change_mode = zjs_cl_flight_legacy=>change_mode-update
      ...
      IF mt_flight_key IS INITIAL.
        SELECT carrier_id, connection_id, flight_date FROM zjs_flight INTO CORRESPONDING FIELDS OF TABLE @mt_flight_key. "#EC CI_NOWHERE
      ENDIF.
      ...
      IF sy-subrc <> 0.
        rv_is_valid = abap_false.
        APPEND NEW zjs_cx_flight_legacy( textid = zjs_cx_flight_legacy=>flight_unknown  carrier_id = is_booking-carrier_id  connection_id = is_booking-connection_id  flight_date = is_booking-flight_date ) TO ct_messages.
      ENDIF.
    ENDIF.
  ENDMETHOD.
  ...
  METHOD _check_currency_code.
    rv_is_valid = abap_true.
    IF   ( iv_change_mode = zjs_cl_flight_legacy=>change_mode-create AND is_booking-currency_code IS NOT INITIAL ) " Will be derived if initial
      OR ( iv_change_mode = zjs_cl_flight_legacy=>change_mode-update AND is_bookingx-currency_code = abap_true ).
  ...
  METHOD _determine.
    IF iv_change_mode = zjs_cl_flight_legacy=>change_mode-create.
      ...
      IF cs_booking-flight_price IS INITIAL OR cs_booking-currency_code IS INITIAL.
        ...
        SELECT SINGLE price, currency_code FROM zjs_flight WHERE carrier_id    = @cs_booking-carrier_id
...
CLASS lcl_travel_buffer DEFINITION FINAL CREATE PRIVATE. "FRIENDS ltc_travel.
  PUBLIC SECTION.
    ...
    METHODS set_status_to_booked IMPORTING iv_travel_id TYPE zjs_de_travel_id
                                 EXPORTING et_messages  TYPE zjs_if_flight_legacy=>tt_if_t100_message.
    METHODS adjust_numbers RETURNING VALUE(rt_travel_mapping) TYPE zjs_if_flight_legacy=>tt_ln_travel_mapping.
    METHODS save.
    METHODS initialize.
    METHODS check_travel_id IMPORTING iv_travel_id       TYPE zjs_de_travel_id
                            CHANGING  ct_messages        TYPE zjs_if_flight_legacy=>tt_if_t100_message
                            RETURNING VALUE(rv_is_valid) TYPE abap_bool.
    "! Prepare changes in a temporary buffer
    "! @parameter iv_no_delete_check | In some cases we do not need to check the existence of a record to be deleted, as this check has been done before.
    "!                               | E.g. delete all subnodes of a node to be deleted.  In this case we have read the subnodes to get their keys.
    METHODS cud_prep IMPORTING it_travel          TYPE zjs_tt_travel
                               it_travelx         TYPE zjs_tt_travelx
                               iv_no_delete_check TYPE abap_bool OPTIONAL
                               iv_numbering_mode  TYPE zjs_if_flight_legacy=>t_numbering_mode DEFAULT zjs_if_flight_legacy=>numbering_mode-early
                     EXPORTING et_travel          TYPE zjs_tt_travel
                               et_messages        TYPE zjs_if_flight_legacy=>tt_if_t100_message.
    "! Add content of the temporary buffer to the real buffer and clear the temporary buffer
    METHODS cud_copy.
    "! Discard content of the temporary buffer
    METHODS cud_disc.
    METHODS get IMPORTING it_travel              TYPE zjs_tt_travel
                          iv_include_buffer      TYPE abap_boolean
                          iv_include_temp_buffer TYPE abap_boolean
                EXPORTING et_travel              TYPE zjs_tt_travel.
  ...
  PRIVATE SECTION.
    ...
    DATA: gv_last_late_number TYPE zjs_de_travel_id VALUE zjs_if_flight_legacy=>late_numbering_boundary.

    " Main buffer
    DATA: mt_create_buffer TYPE zjs_tt_travel,
          mt_update_buffer TYPE zjs_tt_travel,
          mt_delete_buffer TYPE zjs_tt_travel_key.
    " Temporary buffer valid during create / update / delete Travel
    DATA: mt_create_buffer_2 TYPE zjs_tt_travel,
          mt_update_buffer_2 TYPE zjs_tt_travel,
          mt_delete_buffer_2 TYPE zjs_tt_travel_key.

    DATA mt_agency_id   TYPE SORTED TABLE OF zjs_de_agency_id   WITH UNIQUE KEY table_line.

    METHODS _create IMPORTING it_travel         TYPE zjs_tt_travel
                              iv_numbering_mode TYPE zjs_if_flight_legacy=>t_numbering_mode
                    EXPORTING et_travel         TYPE zjs_tt_travel
                              et_messages       TYPE zjs_if_flight_legacy=>tt_if_t100_message.
    METHODS _update IMPORTING it_travel   TYPE zjs_tt_travel
                              it_travelx  TYPE zjs_tt_travelx
                    EXPORTING et_travel   TYPE zjs_tt_travel
                              et_messages TYPE zjs_if_flight_legacy=>tt_if_t100_message.
    METHODS _delete IMPORTING it_travel          TYPE zjs_tt_travel
                              iv_no_delete_check TYPE abap_bool
                    EXPORTING et_messages        TYPE zjs_if_flight_legacy=>tt_if_t100_message.

    METHODS _check IMPORTING is_travel          TYPE zjs_travel
                             is_travelx         TYPE zjs_st_travelx OPTIONAL
                             iv_change_mode     TYPE zjs_cl_flight_legacy=>ty_change_mode
                   CHANGING  ct_messages        TYPE zjs_if_flight_legacy=>tt_if_t100_message
                   RETURNING VALUE(rv_is_valid) TYPE abap_bool.
    METHODS _check_agency IMPORTING is_travel          TYPE zjs_travel
                                    is_travelx         TYPE zjs_st_travelx OPTIONAL
                                    iv_change_mode     TYPE zjs_cl_flight_legacy=>ty_change_mode
                          CHANGING  ct_messages        TYPE zjs_if_flight_legacy=>tt_if_t100_message
                          RETURNING VALUE(rv_is_valid) TYPE abap_bool.
    METHODS _check_customer IMPORTING is_travel          TYPE zjs_travel
                                      is_travelx         TYPE zjs_st_travelx OPTIONAL
                                      iv_change_mode     TYPE zjs_cl_flight_legacy=>ty_change_mode
                            CHANGING  ct_messages        TYPE zjs_if_flight_legacy=>tt_if_t100_message
                            RETURNING VALUE(rv_is_valid) TYPE abap_bool.
    METHODS _check_dates IMPORTING is_travel          TYPE zjs_travel
                                   is_travelx         TYPE zjs_st_travelx OPTIONAL
                                   iv_change_mode     TYPE zjs_cl_flight_legacy=>ty_change_mode
                         CHANGING  ct_messages        TYPE zjs_if_flight_legacy=>tt_if_t100_message
                         RETURNING VALUE(rv_is_valid) TYPE abap_bool.
    METHODS _check_status IMPORTING is_travel          TYPE zjs_travel
                                    is_travelx         TYPE zjs_st_travelx OPTIONAL
                                    iv_change_mode     TYPE zjs_cl_flight_legacy=>ty_change_mode
                          CHANGING  ct_messages        TYPE zjs_if_flight_legacy=>tt_if_t100_message
                          RETURNING VALUE(rv_is_valid) TYPE abap_bool.
    METHODS _check_currency_code IMPORTING is_travel          TYPE zjs_travel
                                           is_travelx         TYPE zjs_st_travelx OPTIONAL
                                           iv_change_mode     TYPE zjs_cl_flight_legacy=>ty_change_mode
                                 CHANGING  ct_messages        TYPE zjs_if_flight_legacy=>tt_if_t100_message
                                 RETURNING VALUE(rv_is_valid) TYPE abap_bool.
    METHODS _update_admin IMPORTING iv_new TYPE abap_bool CHANGING cs_travel_admin TYPE zjs_st_travel_admin.
ENDCLASS.
...
CLASS lcl_travel_buffer IMPLEMENTATION.
  ...
  METHOD _create.
    ...
    DATA lv_travel_id_max     TYPE zjs_de_travel_id.
    ...
    " Get Numbers
    IF iv_numbering_mode EQ zjs_if_flight_legacy=>numbering_mode-late.
      ...
      LOOP AT lt_travel_to_process INTO ls_travel_create ##INTO_OK.
        ...
        ls_travel_create-status = zjs_if_flight_legacy=>travel_status-new.
    ...
    ELSE.
      ...
      IF lv_return_code = '2' OR lv_return_code = '3'.
        APPEND NEW zjs_cx_flight_legacy( textid = zjs_cx_flight_legacy=>travel_nr_last_number ) TO et_messages.
        EXIT.
      ENDIF.
  ...
  METHOD _update.
    ...
    IF sy-subrc = 0.
      APPEND NEW zjs_cx_flight_legacy( textid = zjs_cx_flight_legacy=>travel_no_key ) TO et_messages.
      RETURN.
    ENDIF.

    DATA lt_travel TYPE SORTED TABLE OF zjs_travel WITH UNIQUE KEY travel_id.
    SELECT * FROM zjs_travel FOR ALL ENTRIES IN @it_travel WHERE travel_id = @it_travel-travel_id INTO TABLE @lt_travel ##SELECT_FAE_WITH_LOB[DESCRIPTION].

    FIELD-SYMBOLS <s_buffer_travel> TYPE zjs_travel.
    DATA ls_buffer_travel TYPE zjs_travel.
    LOOP AT it_travel ASSIGNING FIELD-SYMBOL(<s_travel_update>).
      ...
      IF sy-subrc = 0." Error: Record to be updated marked for deletion
        APPEND NEW zjs_cx_flight_legacy( textid = zjs_cx_flight_legacy=>travel_unknown  travel_id = <s_travel_update>-travel_id ) TO et_messages.
        RETURN.
      ENDIF.
      ...
      " Merge fields to be updated
      READ TABLE it_travelx ASSIGNING FIELD-SYMBOL(<s_travelx>) WITH KEY travel_id = <s_travel_update>-travel_id  action_code = zjs_if_flight_legacy=>action_code-update.
      IF sy-subrc <> 0.
        APPEND NEW zjs_cx_flight_legacy( textid = zjs_cx_flight_legacy=>travel_no_control  travel_id = <s_travel_update>-travel_id ) TO et_messages.
        RETURN.
      ENDIF.
      DATA lv_field TYPE i.
      lv_field = 3.
  ...
  METHOD _delete.
    ...
    IF iv_no_delete_check = abap_false.
      DATA lt_travel_db TYPE SORTED TABLE OF zjs_de_travel_id WITH UNIQUE KEY table_line.
      SELECT travel_id FROM zjs_travel FOR ALL ENTRIES IN @lt_travel WHERE travel_id = @lt_travel-travel_id INTO TABLE @lt_travel_db.
    ENDIF.
    ...
    LOOP AT lt_travel ASSIGNING <s_travel_delete>.
      IF iv_no_delete_check = abap_false.
        ...
        IF sy-subrc <> 0.
          APPEND NEW zjs_cx_flight_legacy( textid = zjs_cx_flight_legacy=>travel_unknown  travel_id = <s_travel_delete>-travel_id ) TO et_messages.
          RETURN.
        ENDIF.
      ENDIF.
      INSERT VALUE #( travel_id = <s_travel_delete>-travel_id ) INTO TABLE mt_delete_buffer_2.
  ...
  METHOD set_status_to_booked.
    ...
    IF sy-subrc = 0." Error: Record of action marked for deletion
      APPEND NEW zjs_cx_flight_legacy( textid = zjs_cx_flight_legacy=>travel_unknown  travel_id = iv_travel_id ) TO et_messages.
      RETURN.
    ENDIF.
    ...
    IF <s_travel> IS NOT ASSIGNED.
      SELECT SINGLE * FROM zjs_travel WHERE travel_id = @iv_travel_id INTO @DATA(ls_travel) .
      IF sy-subrc = 0.
    ...
    <s_travel>-status = zjs_if_flight_legacy=>travel_status-booked.
  ...
  METHOD adjust_numbers.
    DATA: lv_is_early_numbering_used TYPE abap_bool VALUE abap_false,
          ...
          lt_create_buffer           TYPE zjs_tt_travel,
          lv_travel_id               TYPE zjs_de_travel_id.
    ...
    IF mt_create_buffer[ 1 ]-travel_id > zjs_if_flight_legacy=>late_numbering_boundary.
      lv_is_late_numbering_used = abap_true.
    ENDIF.
    IF mt_create_buffer[ lines( mt_create_buffer ) ]-travel_id < zjs_if_flight_legacy=>late_numbering_boundary.
  ...
  METHOD save.
    ...
    INSERT zjs_travel FROM TABLE @mt_create_buffer.
    UPDATE zjs_travel FROM TABLE @mt_update_buffer.
    DELETE zjs_travel FROM TABLE @( CORRESPONDING #( mt_delete_buffer ) ).
  ENDMETHOD.
  ...
  METHOD check_travel_id.
    ...
    IF iv_travel_id IS INITIAL.
      APPEND NEW zjs_cx_flight_legacy( textid = zjs_cx_flight_legacy=>travel_no_key ) TO ct_messages.
      RETURN.
    ENDIF.

    IF line_exists( mt_delete_buffer[ travel_id = iv_travel_id ] ).
      APPEND NEW zjs_cx_flight_legacy( textid = zjs_cx_flight_legacy=>travel_unknown  travel_id = iv_travel_id ) TO ct_messages.
      RETURN.
    ENDIF.
    ...
    SELECT SINGLE FROM zjs_travel FIELDS @abap_true WHERE travel_id = @iv_travel_id INTO @DATA(lv_db_exists).
    IF lv_db_exists = abap_true.
      rv_is_valid = abap_true.
      RETURN.
    ENDIF.
  ...
  METHOD cud_prep.
    ...
    DATA lt_travel_c  TYPE zjs_tt_travel.
    DATA lt_travel_u  TYPE zjs_tt_travel.
    DATA lt_travel_d  TYPE zjs_tt_travel.
    DATA lt_travelx_u TYPE zjs_tt_travelx.
    LOOP AT it_travel ASSIGNING FIELD-SYMBOL(<s_travel>).
      ...
      IF sy-subrc <> 0.
        APPEND NEW zjs_cx_flight_legacy( textid     = zjs_cx_flight_legacy=>travel_no_control
                                          travel_id  = <s_travel>-travel_id ) TO et_messages.
        RETURN.
      ENDIF.
      CASE CONV zjs_if_flight_legacy=>action_code_enum( <s_travelx>-action_code ).
        WHEN zjs_if_flight_legacy=>action_code-create.
  ...
  METHOD get.
    ...
    SELECT * FROM zjs_travel FOR ALL ENTRIES IN @it_travel WHERE travel_id = @it_travel-travel_id
  ...
  METHOD _check_agency.
    rv_is_valid = abap_true.
    IF iv_change_mode = zjs_cl_flight_legacy=>change_mode-create OR ( iv_change_mode = zjs_cl_flight_legacy=>change_mode-update AND is_travelx-agency_id = abap_true ).
      IF mt_agency_id IS INITIAL.
        SELECT DISTINCT agency_id FROM zjs_agency INTO TABLE @mt_agency_id. "#EC CI_NOWHERE
      ENDIF.
      READ TABLE mt_agency_id TRANSPORTING NO FIELDS WITH TABLE KEY table_line = is_travel-agency_id.
      IF sy-subrc <> 0.
        rv_is_valid = abap_false.
        APPEND NEW zjs_cx_flight_legacy( textid = zjs_cx_flight_legacy=>agency_unkown  agency_id = is_travel-agency_id ) TO ct_messages.
      ENDIF.
    ENDIF.
  ENDMETHOD.
  ...
  METHOD _check_customer.
    rv_is_valid = abap_true.
    IF iv_change_mode = zjs_cl_flight_legacy=>change_mode-create OR ( iv_change_mode = zjs_cl_flight_legacy=>change_mode-update AND is_travelx-customer_id = abap_true ).
  ...
  METHOD _check_dates.
    ...
    " begin date
    IF iv_change_mode = zjs_cl_flight_legacy=>change_mode-create OR ( iv_change_mode = zjs_cl_flight_legacy=>change_mode-update AND is_travelx-begin_date = abap_true ).
      IF is_travel-begin_date IS INITIAL OR is_travel-begin_date = ''.
        rv_is_valid = abap_false.
        APPEND NEW zjs_cx_flight_legacy( textid = zjs_cx_flight_legacy=>no_begin_date  travel_id = is_travel-travel_id ) TO ct_messages.
      ENDIF.
    ENDIF.

    " end date
    IF iv_change_mode = zjs_cl_flight_legacy=>change_mode-create OR ( iv_change_mode = zjs_cl_flight_legacy=>change_mode-update AND is_travelx-end_date = abap_true ).
      IF is_travel-end_date IS INITIAL OR is_travel-end_date = ''.
        rv_is_valid = abap_false.
        APPEND NEW zjs_cx_flight_legacy( textid = zjs_cx_flight_legacy=>no_end_date  travel_id = is_travel-travel_id ) TO ct_messages.
      ENDIF.
    ENDIF.

    " begin date < = end date
    IF iv_change_mode = zjs_cl_flight_legacy=>change_mode-create OR ( iv_change_mode = zjs_cl_flight_legacy=>change_mode-update AND is_travelx-begin_date = abap_true )
                                                                  OR ( iv_change_mode = zjs_cl_flight_legacy=>change_mode-update AND is_travelx-end_date   = abap_true ).
      IF is_travel-begin_date IS NOT INITIAL AND is_travel-end_date IS NOT INITIAL.
        IF is_travel-begin_date > is_travel-end_date.
          rv_is_valid = abap_false.
          APPEND NEW zjs_cx_flight_legacy( textid = zjs_cx_flight_legacy=>end_date_before_begin_date  begin_date = is_travel-begin_date  end_date = is_travel-end_date  travel_id = is_travel-travel_id ) TO ct_messages.
        ENDIF.
      ENDIF.
    ENDIF.
  ENDMETHOD.
  ...
  METHOD _check_status.
    rv_is_valid = abap_true.
    IF iv_change_mode = zjs_cl_flight_legacy=>change_mode-update AND is_travelx-status = abap_true.
      CASE is_travel-status.
        WHEN CONV zjs_de_travel_status( zjs_if_flight_legacy=>travel_status-booked ).   " OK
        WHEN CONV zjs_de_travel_status( zjs_if_flight_legacy=>travel_status-cancelled )." OK
        WHEN CONV zjs_de_travel_status( zjs_if_flight_legacy=>travel_status-new ).      " OK
        WHEN CONV zjs_de_travel_status( zjs_if_flight_legacy=>travel_status-planned ).  " OK
        WHEN OTHERS.
          rv_is_valid = abap_false.
          APPEND NEW zjs_cx_flight_legacy( textid = zjs_cx_flight_legacy=>travel_status_invalid  status = is_travel-status ) TO ct_messages.
      ENDCASE.
    ENDIF.
  ENDMETHOD.
  ...
  METHOD _check_currency_code.
    ...
    ENDIF.
    IF   iv_change_mode = zjs_cl_flight_legacy=>change_mode-create
      OR ( iv_change_mode = zjs_cl_flight_legacy=>change_mode-update AND is_travelx-currency_code = abap_true ).
