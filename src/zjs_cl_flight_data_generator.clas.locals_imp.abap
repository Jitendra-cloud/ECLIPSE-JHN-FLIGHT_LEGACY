...
CLASS lcl_agency_data_generator DEFINITION CREATE PRIVATE.
  ...
  PUBLIC SECTION.
    ...
    TYPES: tt_agency TYPE STANDARD TABLE OF zjs_agency WITH KEY agency_id.
  ...
  PRIVATE SECTION.
    CONSTANTS:
      ...
      cv_numberrange_object   TYPE cl_numberrange_runtime=>nr_object   VALUE 'ZJS_AGNCY' ##NO_TEXT,
      cv_agency_minimum       TYPE zjs_de_agency_id VALUE '070001',
      cv_agency_maximum       TYPE zjs_de_agency_id VALUE '079999'.
...
CLASS lcl_agency_data_generator IMPLEMENTATION.
  ...
  METHOD lif_data_generator~create.
    ...
    ENDIF.
    DELETE FROM zjs_agency.                             "#EC CI_NOWHERE
    ...
    ENDIF.
    INSERT zjs_agency FROM TABLE @lt_data.
  ...
  METHOD set_numberrange.
    ...
    zjs_cl_flight_data_generator=>reset_numberrange_interval(
...
CLASS lcl_airport_data_generator DEFINITION CREATE PRIVATE.
  ...
  PUBLIC SECTION.
    INTERFACES: lif_data_generator.
    TYPES: tt_airport TYPE STANDARD TABLE OF zjs_airport WITH KEY airport_id.
    CLASS-METHODS: get_data
...
CLASS lcl_airport_data_generator IMPLEMENTATION.
  ...
  METHOD lif_data_generator~create.
    IF out IS BOUND.  out->write( '--> Delete Content.' ) ##NO_TEXT.
    ENDIF.
    DELETE FROM zjs_airport.                            "#EC CI_NOWHERE
    ...
    IF out IS BOUND.  out->write( '--> Insert Content.' ) ##NO_TEXT.
    ENDIF.
    INSERT zjs_airport FROM TABLE @lt_data.
...
CLASS lcl_carrier_data_generator DEFINITION CREATE PRIVATE.
  ...
  PUBLIC SECTION.
    INTERFACES: lif_data_generator.
    TYPES: tt_carrier TYPE STANDARD TABLE OF zjs_carrier WITH KEY carrier_id.
    CLASS-METHODS: get_data
      RETURNING
...
CLASS lcl_carrier_data_generator IMPLEMENTATION.
  ...
  METHOD lif_data_generator~create.
    IF out IS BOUND.  out->write( '--> Delete Content.' ) ##NO_TEXT.
    ENDIF.
    DELETE FROM zjs_carrier.                            "#EC CI_NOWHERE
    ...
    IF out IS BOUND.  out->write( '--> Insert Content.' ) ##NO_TEXT.
    ENDIF.
    INSERT zjs_carrier FROM TABLE @lt_data.
...
CLASS lcl_connection_data_generator DEFINITION CREATE PRIVATE.
  ...
  PUBLIC SECTION.
    INTERFACES: lif_data_generator.
    TYPES: tt_connection TYPE STANDARD TABLE OF zjs_connection WITH KEY carrier_id connection_id.
...
CLASS lcl_connection_data_generator IMPLEMENTATION.
  ...
  METHOD lif_data_generator~create.
    IF out IS BOUND.  out->write( '--> Delete Content.' ) ##NO_TEXT.
    ENDIF.
    DELETE FROM zjs_connection.                         "#EC CI_NOWHERE
    ...
    IF out IS BOUND.  out->write( '--> Insert Content.' ) ##NO_TEXT.
    ENDIF.
    INSERT zjs_connection FROM TABLE @lt_data.
...
CLASS zjs_cl_flight_data_generator DEFINITION LOCAL FRIENDS lcl_flight_data_generator.
CLASS lcl_flight_data_generator DEFINITION CREATE PRIVATE.

  PUBLIC SECTION.
    INTERFACES: lif_data_generator.
    TYPES: tt_flights TYPE STANDARD TABLE OF zjs_flight
  ...
  PRIVATE SECTION.
    TYPES: BEGIN OF ty_plane_type,
             id            TYPE zjs_de_plane_type_id,
             seats_max     TYPE zjs_de_plane_seats_max,
           ...
           BEGIN OF ty_flight_info,
             ...
             long_distance  TYPE abap_bool,
             seats_max      TYPE zjs_de_plane_seats_max,
             seats_occupied TYPE zjs_de_plane_seats_occupied,
             price          TYPE zjs_de_flight_price,
           END OF ty_flight_info,
           BEGIN OF ty_connection_recurrency,
             id         TYPE zjs_de_connection_id,
             recurrency TYPE STANDARD TABLE OF zjs_de_flight_date WITH EMPTY KEY,
    ...
    CLASS-METHODS: build_dependent_content,
      get_flight_info
        IMPORTING
          iv_connection_id     TYPE zjs_de_connection_id
        RETURNING
...
CLASS lcl_flight_data_generator IMPLEMENTATION.
  ...
  METHOD lif_data_generator~create.
    IF out IS BOUND.  out->write( '--> Delete Content.' ) ##NO_TEXT.
    ENDIF.
    DELETE FROM zjs_flight.                             "#EC CI_NOWHERE
    ...
    IF out IS BOUND.  out->write( '--> Insert Content.' ) ##NO_TEXT.
    ENDIF.
    INSERT zjs_flight FROM TABLE @gt_flights.
    ...
    LOOP AT gt_connections INTO DATA(ls_connection).
      LOOP AT gt_connection_recurrency[ id = ls_connection-connection_id ]-recurrency INTO DATA(lv_flight_date).
        ...
        APPEND VALUE zjs_flight(
                  ...
                  currency_code  = VALUE zjs_flight-currency_code(
  ...
  METHOD get_flight_info.
    ...
    rs_plane_info = VALUE ty_flight_info(
        ...
        price          = zjs_cl_flight_data_generator=>calculate_flight_price(
  ...
  METHOD build_connection_recurrency.
    ...
    flight_date_max = calc_next_monday( CONV zjs_de_flight_date( lv_datum + 217 ) ).

    " flight_date_min is a Monday 5 months in the past
    flight_date_min = calc_next_monday( CONV zjs_de_flight_date( lv_datum - 148 ) ).
      ...
      APPEND VALUE ty_connection_recurrency(
          ...
          recurrency = VALUE ty_connection_recurrency-recurrency(
                                    ...
                                    UNTIL flightdate < flight_date_min + ls_connection-weekday - 1
                                       ( CONV zjs_de_flight_date( flightdate ) )
                               )
      ) TO rt_data.
    ENDLOOP.
  ENDMETHOD.
...
CLASS lcl_customer_data_generator DEFINITION CREATE PRIVATE.
  ...
  PUBLIC SECTION.
    ...
    TYPES: BEGIN OF ty_last_name,
             last_name TYPE zjs_de_last_name,
           END OF ty_last_name.
    TYPES: tt_customer  TYPE STANDARD TABLE OF zjs_customer WITH KEY customer_id,
  ...
  PRIVATE SECTION.
    TYPES:
      ...
      BEGIN OF ty_first_name,
        first_name TYPE zjs_de_first_name,
        gender     TYPE c LENGTH 1,
      END OF ty_first_name,
      BEGIN OF ty_name,
        first_name TYPE zjs_de_first_name,
        last_name  TYPE zjs_de_last_name,
        title      TYPE zjs_de_title,
      END OF ty_name,
      " Addresses
      BEGIN OF ty_city,
        country     TYPE land1,
        postal_code TYPE zjs_de_postal_code,
        city        TYPE zjs_de_city,
      END OF ty_city,
      tt_street_per_country TYPE STANDARD TABLE OF zjs_de_street WITH EMPTY KEY,
      BEGIN OF ty_street,
        country TYPE land1,
      ...
      BEGIN OF ty_address,
        ...
        postal_code   TYPE zjs_de_postal_code,
        city          TYPE zjs_de_city,
        street        TYPE zjs_de_street,
        phone_number  TYPE  zjs_de_phone_number,
        email_address TYPE zjs_de_email_address,
      END OF ty_address.
...
CLASS lcl_customer_data_generator IMPLEMENTATION.
  ...
  METHOD lif_data_generator~create.
    IF out IS BOUND.  out->write( '--> Delete Content.' ) ##NO_TEXT.
    ENDIF.
    DELETE FROM zjs_customer.                           "#EC CI_NOWHERE
    ...
    IF out IS BOUND.  out->write( '--> Insert Content.' ) ##NO_TEXT.
    ENDIF.
    INSERT zjs_customer FROM TABLE @lt_data.
    ...
    LOOP AT build_last_names( ) INTO DATA(ls_last_name).
      DO lo_random_counter->get_next( ) TIMES.
        ...
        APPEND VALUE ty_name(
            ...
            title      = COND zjs_de_title( WHEN ls_first_name-gender = 'M' THEN 'Mr.'
...
CLASS lcl_supplement_data_generator DEFINITION CREATE PRIVATE.
  ...
  PUBLIC SECTION.
    INTERFACES: lif_data_generator.
    TYPES:
      tt_supplement               TYPE STANDARD TABLE OF zjs_supplement  WITH KEY supplement_id,
      tt_supplement_text          TYPE STANDARD TABLE OF zjs_suppl_text  WITH KEY supplement_id language_code,
      tt_supplement_category      TYPE STANDARD TABLE OF zjs_supplcat    WITH KEY supplement_category,
      tt_supplement_category_text TYPE STANDARD TABLE OF zjs_supplcat_t  WITH KEY supplement_category language_code.

    " Merged types
    TYPES BEGIN OF ty_supplement_complete.
    INCLUDE TYPE zjs_supplement.
    TYPES language_code TYPE spras.
    TYPES description   TYPE zjs_de_description.
    TYPES END OF ty_supplement_complete.

    TYPES tt_supplement_category_compl TYPE STANDARD TABLE OF zjs_supplcat_t WITH KEY supplement_category language_code.
  ...
  PRIVATE SECTION.
    ...
    CONSTANTS:
      BEGIN OF cs_supplement_category,
        beverage TYPE zjs_de_supplement_category VALUE 'BV',
        meal     TYPE zjs_de_supplement_category VALUE 'ML',
        luggage  TYPE zjs_de_supplement_category VALUE 'LU',
        extra    TYPE zjs_de_supplement_category VALUE 'EX',
      END OF cs_supplement_category.

    CONSTANTS:
      cv_numberrange_interval TYPE cl_numberrange_runtime=>nr_interval VALUE '01',
      cv_numberrange_object   TYPE cl_numberrange_runtime=>nr_object   VALUE 'ZJS_SUPPL' ##NO_TEXT.
...
CLASS lcl_supplement_data_generator IMPLEMENTATION.
  ...
  METHOD lif_data_generator~create.
    ...
    ENDIF.
    DELETE FROM:
      zjs_supplement,                                   "#EC CI_NOWHERE
      zjs_suppl_text,                                   "#EC CI_NOWHERE
      zjs_supplcat,                                     "#EC CI_NOWHERE
      zjs_supplcat_t.                                   "#EC CI_NOWHERE
    ...
    INSERT zjs_supplcat    FROM TABLE @( CORRESPONDING tt_supplement_category(      lt_type ) ).
    INSERT zjs_supplcat_t  FROM TABLE @( CORRESPONDING tt_supplement_category_text( lt_type ) ).

*   The number range has to be set after inserting values into zjs_supplcat as it reads the data from the table to validate the sub-objects ...
    ...
    IF out IS BOUND.  out->write( '--> Insert Content.' ) ##NO_TEXT.
    ENDIF.
    INSERT zjs_supplement  FROM TABLE @( CORRESPONDING tt_supplement(               lt_data ) ).
    INSERT zjs_suppl_text  FROM TABLE @( CORRESPONDING tt_supplement_text(          lt_data ) ).
    IF out IS BOUND.  out->write( '--> Done.' ) ##NO_TEXT.
    ENDIF.
    IF out IS BOUND.  out->write( '--> Done.' ) ##NO_TEXT.
    ENDIF.
...
CLASS lcl_status_vh_data_generator DEFINITION CREATE PRIVATE.
  ...
  PRIVATE SECTION.
    TYPES:
      type_travel_status  TYPE STANDARD TABLE OF zjs_trvl_stat_t WITH KEY travel_status  language,
      type_overall_status TYPE STANDARD TABLE OF zjs_oall_stat_t WITH KEY overall_status language,
      type_booking_status TYPE STANDARD TABLE OF zjs_book_stat_t WITH KEY booking_status language.
...
CLASS lcl_status_vh_data_generator IMPLEMENTATION.
  ...
  METHOD lif_data_generator~create.
    ...
    ENDIF.
    DELETE FROM:
       zjs_trvl_stat,                                   "#EC CI_NOWHERE
       zjs_oall_stat,                                   "#EC CI_NOWHERE
       zjs_book_stat,                                   "#EC CI_NOWHERE
       zjs_trvl_stat_t,                                 "#EC CI_NOWHERE
       zjs_oall_stat_t,                                 "#EC CI_NOWHERE
       zjs_book_stat_t.                                 "#EC CI_NOWHERE


    IF out IS BOUND.  out->write( '--> Travel Status' ) ##no_text.
    ENDIF.
    DATA(travel_status) = _travel_status( ).
    INSERT zjs_trvl_stat   FROM TABLE @( CORRESPONDING #( travel_status ) ).
    INSERT zjs_trvl_stat_t FROM TABLE @travel_status.

    IF out IS BOUND.  out->write( '--> Overall Status' ) ##no_text.
    ENDIF.
    DATA(overall_status) = _overall_status( ).
    INSERT zjs_oall_stat   FROM TABLE @( CORRESPONDING #( overall_status ) ).
    INSERT zjs_oall_stat_t FROM TABLE @overall_status.

    IF out IS BOUND.  out->write( '--> Booking Status' ) ##no_text.
    ENDIF.
    DATA(booking_status) = _booking_status( ).
    INSERT zjs_book_stat   FROM TABLE @( CORRESPONDING #( booking_status ) ).
    INSERT zjs_book_stat_t FROM TABLE @booking_status.
...
CLASS zjs_cl_flight_data_generator DEFINITION LOCAL FRIENDS lcl_travel_data_generator.
CLASS lcl_travel_data_generator DEFINITION CREATE PRIVATE.

  PUBLIC SECTION.
    INTERFACES: lif_data_generator.

    TYPES: tt_travel              TYPE STANDARD TABLE OF zjs_travel     WITH KEY travel_id,
           tt_bookings            TYPE STANDARD TABLE OF zjs_booking    WITH KEY travel_id booking_id,
           tt_booking_supplements TYPE STANDARD TABLE OF zjs_book_suppl WITH KEY travel_id booking_id booking_supplement_id.

    " Build nested tables
    TYPES BEGIN OF ty_booking_complete.
    INCLUDE TYPE zjs_booking.
  ...
  PRIVATE SECTION.
    ...
    CONSTANTS:
      ...
      cv_numberrange_interval        TYPE cl_numberrange_runtime=>nr_interval VALUE '01',
      cv_numberrange_object          TYPE cl_numberrange_runtime=>nr_object   VALUE 'ZJS_TRAVL' ##NO_TEXT.
    ...
    CLASS-METHODS: get_data
      ...
      build_booking
        IMPORTING
          iv_travel_id       TYPE zjs_booking-travel_id
        RETURNING
      ...
      find_next_fitting_flight
        IMPORTING
          ...
          is_flight_previous TYPE zjs_flight OPTIONAL
        RETURNING
          VALUE(rs_flight)   TYPE zjs_flight,
      generate_booking_supplements
        IMPORTING
          iv_travel_id   TYPE zjs_booking-travel_id
          iv_booking_id  TYPE zjs_booking-booking_id
        RETURNING
      ...
      generate_description
        ...
        RETURNING
          VALUE(rv_description) TYPE zjs_travel-description,
      ...
      calculate_booking_fee
        IMPORTING
          it_bookings           TYPE lcl_travel_data_generator=>tt_booking_complete
        RETURNING
          VALUE(rv_booking_fee) TYPE zjs_travel-booking_fee,
      generate_travel_customer_id
        IMPORTING
          it_bookings           TYPE lcl_travel_data_generator=>tt_booking_complete
        RETURNING
          VALUE(rv_customer_id) TYPE zjs_travel-customer_id,
      set_today,
...
CLASS lcl_travel_data_generator IMPLEMENTATION.
  ...
  METHOD lif_data_generator~create.
    ...
    LOOP AT lt_data INTO DATA(ls_travel).
      APPEND CORRESPONDING zjs_travel( ls_travel ) TO lt_travels.
      LOOP AT ls_travel-bookings INTO DATA(ls_booking).
        APPEND CORRESPONDING zjs_booking( ls_booking ) TO lt_bookings.
    ...
    ENDIF.
    INSERT zjs_travel FROM TABLE @lt_travels.
    IF out IS BOUND.  out->write( '--> Insert Booking Content' ) ##NO_TEXT.
    ENDIF.
    INSERT zjs_booking FROM TABLE @lt_bookings.
    IF out IS BOUND.  out->write( '--> Insert Booking Supplement Content' ) ##NO_TEXT.
    ENDIF.
    INSERT zjs_book_suppl FROM TABLE @lt_booking_supplements.
  ...
  METHOD get_data.
    DATA: lv_travel_id TYPE zjs_booking-travel_id.
    ...
    WHILE lt_bookings IS NOT INITIAL.
      ...
      APPEND VALUE ty_travel_complete(
          ...
          total_price   = lv_booking_fee + REDUCE zjs_travel-total_price( INIT sum = 0
                                                          FOR booking IN lt_bookings
                                                          NEXT
                                                           sum = sum
                                                                 + booking-flight_price
                                                                 + REDUCE zjs_de_flight_price( INIT sum_supplement = 0
          ...
          status        = SWITCH zjs_travel-status( go_ran_status_description->get_next( )
                                          WHEN 1 OR 2 THEN zjs_if_flight_legacy=>travel_status-new
                                          WHEN 3      THEN zjs_if_flight_legacy=>travel_status-booked
                                          WHEN 4      THEN zjs_if_flight_legacy=>travel_status-planned  )
  ...
  METHOD set_numberrange.
    ...
    zjs_cl_flight_data_generator=>reset_numberrange_interval(
      EXPORTING
        ...
        tonumber             = CONV cl_numberrange_intervals=>nr_nriv_line-tonumber( CONV zjs_de_travel_id( zjs_if_flight_legacy=>late_numbering_boundary - 1 ) ) ).
  ...
  METHOD build_booking.
    TYPES: tt_customer_id TYPE TABLE OF zjs_customer-customer_id WITH EMPTY KEY.
    DATA: lv_booking_id      TYPE zjs_booking-booking_id,
    ...
    DO lv_trip_length TIMES.
      ...
      DATA(lv_booking_date) = CONV zjs_booking-booking_date( <flight>-flight_date - go_ran_booking_date->get_next( ) ).

      DATA(lv_price) = zjs_cl_flight_data_generator=>calculate_flight_price(
      ...
      FOR i = 1  THEN i + 1  WHILE i <= lines( lt_customer_id )
         (
            ...
            booking_supplements = generate_booking_supplements( iv_travel_id = iv_travel_id   iv_booking_id = CONV zjs_booking-booking_id( lv_booking_id + i ) )
          )
      ) TO rt_bookings.
  ...
  METHOD find_next_fitting_flight.
    ...
    IF is_flight_previous IS SUPPLIED.
      DATA(lv_connection_id_new) = VALUE zjs_connection-connection_id( gt_connections[
  ...
  METHOD generate_description.
    TYPES: tt_customers TYPE SORTED TABLE OF zjs_customer WITH UNIQUE KEY customer_id.
    rv_description = SWITCH zjs_travel-description(
                               go_ran_travel_description->get_next( )
                                  WHEN 1 THEN `Business Trip for ` &&
                                                REDUCE zjs_travel-description(
                                                               ...
                                                               NEXT s =  s && gt_customer[ customer_id = traveler-customer_id ]-first_name
                                                                           && COND zjs_travel-description( WHEN i < lines( it_bookings ) THEN `, ` )
                                                                    i = i + 1 )
  ...
  METHOD generate_travel_customer_id.
    rv_customer_id = COND zjs_travel-customer_id(
